...
- First-kind tetrahedral Nédélec reference bases through order four have
  ...
  polynomials and analytical curls are generated by pinned `fortsym` code.
  The affine covariant map and local curl-mass assembly accept every
  implemented order. At first order, neighboring tetrahedra share global edge
  degrees of freedom with explicit local orientation signs, and the global
