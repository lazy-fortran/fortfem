module fortfem_helmholtz_panel_pair_3d
    use fortfem_generated_helmholtz_single_layer_smooth_integrand_jvp, only: &
        generated_helmholtz_single_layer_smooth_integrand_jvp
    use fortfem_generated_helmholtz_single_layer_smooth_integrand_vjp, only: &
        generated_helmholtz_single_layer_smooth_integrand_vjp
...
contains
    ...
    subroutine assemble_helmholtz_single_layer_p0_3d_jvp( &
        ...
        call assemble_laplace_single_layer_p0_3d_jvp( &
            vertices, triangles, quadrature_degree, vertices_dot, &
            laplace_matrix_dot, status)
        if (status /= 0) return
        allocate(matrix_dot(size(laplace_matrix_dot, 1), &
            size(laplace_matrix_dot, 2)))
        matrix_dot = cmplx(laplace_matrix_dot, 0.0_dp, dp)
        do first = 1, size(triangles, 2)
            ...
            do second = 1, first
                ...
                call integrate_helmholtz_single_layer_smooth_panel_pair_jvp( &
                    first_vertices, second_vertices, wave_number, &
                    quadrature_degree, first_vertices_dot, &
                    second_vertices_dot, wave_number_dot, value_dot, status)
                if (status /= 0) return
                matrix_dot(first, second) = &
                    matrix_dot(first, second) + value_dot
                matrix_dot(second, first) = matrix_dot(first, second)
            end do
        end do
        status = 0
    end subroutine assemble_helmholtz_single_layer_p0_3d_jvp

    subroutine assemble_helmholtz_single_layer_p0_3d_vjp( &
        ...
        real(dp) :: laplace_matrix_bar(size(matrix_bar, 1), size(matrix_bar, 2))
        ...
        laplace_matrix_bar = real(matrix_bar, dp)
        call assemble_laplace_single_layer_p0_3d_vjp( &
            vertices, triangles, quadrature_degree, laplace_matrix_bar, &
            vertices_bar, status)
        if (status /= 0) return
        do first = 1, size(triangles, 2)
            first_vertices = vertices(:, triangles(:, first))
            do second = 1, first
                ...
                if (first == second) then
                    value_bar = matrix_bar(first, first)
                else
                    value_bar = matrix_bar(first, second) + &
                        matrix_bar(second, first)
                end if
                call integrate_helmholtz_single_layer_smooth_panel_pair_vjp( &
                    first_vertices, second_vertices, wave_number, &
                    quadrature_degree, value_bar, first_vertices_bar, &
                    second_vertices_bar, local_wave_number_bar, status)
                if (status /= 0) return
                do local = 1, 3
        ...
        status = 0
    end subroutine assemble_helmholtz_single_layer_p0_3d_vjp

    subroutine integrate_helmholtz_single_layer_smooth_panel_pair_jvp( &
        ...
        real(dp) :: radius, second_jacobian, second_jacobian_dot
        real(dp) :: second_point(3), second_point_dot(3)
        real(dp) :: term_real_dot, term_imag_dot
        ...
        do first_node = 1, size(weights)
            ...
            do second_node = 1, size(weights)
                ...
                if (radius <= 64.0_dp*epsilon(1.0_dp)) then
                    term_real_dot = 0.0_dp
                    term_imag_dot = kernel_scale*( &
                        wave_number_dot*first_jacobian*second_jacobian + &
                        wave_number*first_jacobian_dot*second_jacobian + &
                        wave_number*first_jacobian*second_jacobian_dot)
                else
                    call &
                        generated_helmholtz_single_layer_smooth_integrand_jvp( &
                        first_point(1), first_point(2), first_point(3), &
                        second_point(1), second_point(2), second_point(3), &
                        first_jacobian, second_jacobian, wave_number, &
                        kernel_scale, first_point_dot(1), first_point_dot(2), &
                        first_point_dot(3), second_point_dot(1), &
                        second_point_dot(2), second_point_dot(3), &
                        first_jacobian_dot, second_jacobian_dot, &
                        wave_number_dot, term_real_dot, term_imag_dot)
                end if
                value_dot = value_dot + &
        ...
        status = 0
    end subroutine integrate_helmholtz_single_layer_smooth_panel_pair_jvp

    subroutine integrate_helmholtz_single_layer_smooth_panel_pair_vjp( &
            first_vertices, second_vertices, wave_number, quadrature_degree, &
        ...
        real(dp) :: first_jacobian, first_point(3), kernel_scale, radius
        ...
        do first_node = 1, size(weights)
            ...
            do second_node = 1, size(weights)
                ...
                radius = norm2(first_point - second_point)
                if (radius <= 64.0_dp*epsilon(1.0_dp)) then
                    local_first_point_bar = 0.0_dp
                    local_second_point_bar = 0.0_dp
                    local_first_jacobian_bar = aimag(value_bar)*kernel_scale* &
                        wave_number*second_jacobian
                    local_second_jacobian_bar = aimag(value_bar)*kernel_scale* &
                        wave_number*first_jacobian
                    local_wave_number_bar = aimag(value_bar)*kernel_scale* &
                    ...
                    call &
                        ...
                        second_point(1), second_point(2), second_point(3), &
                        first_jacobian, second_jacobian, wave_number, &
                        kernel_scale, real(value_bar, dp), aimag(value_bar), &
                        ...
                        local_first_jacobian_bar, &
                        local_second_jacobian_bar, local_wave_number_bar)
                end if
        ...
        status = 0
    end subroutine integrate_helmholtz_single_layer_smooth_panel_pair_vjp
