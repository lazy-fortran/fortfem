module fortfem_helmholtz_sphere_panel_pair_ad_3d
...
contains
    ...
    subroutine integrate_helmholtz_sphere_panel_p0_3d( &
            first_vertices, second_vertices, radius, wave_number, &
        ...
        real(dp) :: first_tangent_eta(3), first_tangent_xi(3)
        real(dp) :: second_tangent_eta(3), second_tangent_xi(3)
        ...
        status = 0
    end subroutine integrate_helmholtz_sphere_panel_p0_3d

    subroutine integrate_helmholtz_sphere_panel_p0_3d_jvp( &
            first_vertices, second_vertices, radius, wave_number, &
        ...
        real(dp), intent(in) :: radius_dot, wave_number_dot
        ...
        real(dp) :: second_tangent_xi_dot(3), term_real_dot, term_imag_dot
        integer :: first_node, quadrature_status, second_node
        ...
        do first_node = 1, size(weights)
            ...
            do second_node = 1, size(weights)
                ...
                call generated_helmholtz_single_layer_integrand_jvp( &
                    ...
                    second_jacobian_dot, wave_number_dot, term_real_dot, &
                    term_imag_dot)
                value_dot = value_dot + cmplx(term_real_dot, term_imag_dot, dp)
            end do
        end do
        status = 0
    end subroutine integrate_helmholtz_sphere_panel_p0_3d_jvp

    subroutine integrate_helmholtz_sphere_panel_p0_3d_vjp( &
            first_vertices, second_vertices, radius, wave_number, &
            quadrature_degree, value_bar, first_vertices_bar, &
            second_vertices_bar, radius_bar, wave_number_bar, status)
        real(dp), intent(in) :: first_vertices(3, 3), second_vertices(3, 3)
        real(dp), intent(in) :: radius, wave_number
        integer, intent(in) :: quadrature_degree
        ...
        real(dp), intent(out) :: radius_bar, wave_number_bar
        integer, intent(out) :: status
        ...
        real(dp) :: local_first_vertices_bar(3, 3)
        real(dp) :: local_second_vertices_bar(3, 3)
        real(dp) :: local_radius_bar, local_wave_number_bar
        real(dp) :: xi_bar, eta_bar, term_real, term_imag
        integer :: first_node, quadrature_status, second_node
        ...
        do first_node = 1, size(weights)
            ...
            do second_node = 1, size(weights)
                ...
                call generated_helmholtz_single_layer_integrand_vjp( &
                    ...
                    second_point(1), second_point(2), second_point(3), &
                    first_jacobian, second_jacobian, wave_number, kernel_scale, &
                    real(value_bar, dp), aimag(value_bar), first_point_bar(1), &
                    first_point_bar(2), first_point_bar(3), second_point_bar(1), &
                    second_point_bar(2), second_point_bar(3), first_jacobian_bar, &
                    second_jacobian_bar, local_wave_number_bar)
                call evaluate_sphere_curved_panel_vjp( &
                    first_vertices, radius, xi(first_node), eta(first_node), &
                    first_point_bar, [0.0_dp, 0.0_dp, 0.0_dp], &
                    [0.0_dp, 0.0_dp, 0.0_dp], first_jacobian_bar, &
                    local_first_vertices_bar, local_radius_bar, xi_bar, &
                    eta_bar, status)
                if (status /= 0) return
                first_vertices_bar = first_vertices_bar + local_first_vertices_bar
                radius_bar = radius_bar + local_radius_bar
                call evaluate_sphere_curved_panel_vjp( &
                    second_vertices, radius, xi(second_node), eta(second_node), &
                    second_point_bar, [0.0_dp, 0.0_dp, 0.0_dp], &
                    [0.0_dp, 0.0_dp, 0.0_dp], second_jacobian_bar, &
                    local_second_vertices_bar, local_radius_bar, xi_bar, &
                    eta_bar, status)
                if (status /= 0) return
                second_vertices_bar = second_vertices_bar + local_second_vertices_bar
                radius_bar = radius_bar + local_radius_bar
                wave_number_bar = wave_number_bar + local_wave_number_bar
            end do
        end do
        status = 0
    end subroutine integrate_helmholtz_sphere_panel_p0_3d_vjp

end module fortfem_helmholtz_sphere_panel_pair_ad_3d
