module fortfem_patch_graph_complex_trace_contraction
    !! Complex weighted cycle contractions on an oriented patch graph.
    !!
    !! The graph supplies a neutral interface-sample ledger and an integer
    !! cycle basis.  Complex trace components are contracted without selecting
    !! a field, discretization, or Fourier convention.  The reverse product
    !! is the adjoint under the real part of the complex Euclidean pairing.
...
contains
    ...
    subroutine assemble_patch_graph_complex_trace_contraction( &
            graph, trace_values, cycle_contraction, status)
        type(boundary_region_graph_t), intent(in) :: graph
        complex(dp), intent(in) :: trace_values(:, :)
        complex(dp), allocatable, intent(out) :: cycle_contraction(:, :)
        ...
        if (.not. valid_complex_shape( &
                trace_values, component_count, sample_count)) then
            allocate(cycle_contraction(component_count, 0))
            status = 2
            return
        end if
        allocate(cycle_contraction(component_count, cycle_count))
        call apply_complex_contraction( &
            cycle_basis, edge_offsets, all_weights, trace_values, &
            cycle_contraction)
        status = 0
    end subroutine assemble_patch_graph_complex_trace_contraction

    subroutine assemble_patch_graph_complex_trace_contraction_jvp( &
            graph, trace_values_dot, cycle_contraction_dot, status)
        type(boundary_region_graph_t), intent(in) :: graph
        complex(dp), intent(in) :: trace_values_dot(:, :)
        complex(dp), allocatable, intent(out) :: cycle_contraction_dot(:, :)
        integer, intent(out) :: status
        integer, allocatable :: cycle_basis(:, :), edge_offsets(:)
        real(dp), allocatable :: all_weights(:)
        integer :: component_count, cycle_count, sample_count
        ...
        status = 0
    end subroutine assemble_patch_graph_complex_trace_contraction_jvp

    subroutine assemble_patch_graph_complex_trace_contraction_vjp( &
            graph, cycle_contraction_bar, trace_values_bar, status)
        type(boundary_region_graph_t), intent(in) :: graph
        complex(dp), intent(in) :: cycle_contraction_bar(:, :)
        complex(dp), allocatable, intent(out) :: trace_values_bar(:, :)
        integer, intent(out) :: status
        integer, allocatable :: cycle_basis(:, :), edge_offsets(:)
        real(dp), allocatable :: all_weights(:)
        integer :: component_count, cycle_count, sample_count
        ...
        call apply_complex_contraction_vjp( &
            cycle_basis, edge_offsets, all_weights, cycle_contraction_bar, &
            trace_values_bar)
        status = 0
    end subroutine assemble_patch_graph_complex_trace_contraction_vjp

    subroutine prepare_complex_ledger( &
            graph, cycle_basis, edge_offsets, all_weights, status)
        type(boundary_region_graph_t), intent(in) :: graph
        ...
        do edge = 1, edge_count
            ...
            if (sample_count > 0) then
                all_weights(total_samples + 1:total_samples + sample_count) = &
                    weights
            end if
            total_samples = total_samples + sample_count
        end do
        status = 0
    end subroutine prepare_complex_ledger

    subroutine apply_complex_contraction( &
            cycle_basis, edge_offsets, all_weights, values, contraction)
        integer, intent(in) :: cycle_basis(:, :), edge_offsets(:)
        real(dp), intent(in) :: all_weights(:)
        complex(dp), intent(in) :: values(:, :)
        complex(dp), intent(out) :: contraction(:, :)
        integer :: edge, cycle_id, component, first_sample, last_sample

        contraction = cmplx(0.0_dp, 0.0_dp, dp)
        do edge = 1, size(cycle_basis, 1)
            ...
            do cycle_id = 1, size(cycle_basis, 2)
                do component = 1, size(values, 1)
                    contraction(component, cycle_id) = &
                        contraction(component, cycle_id) + &
                        real(cycle_basis(edge, cycle_id), dp)*sum( &
                        all_weights(first_sample:last_sample)* &
                        values(component, first_sample:last_sample))
                end do
            end do
        end do
    end subroutine apply_complex_contraction

    subroutine apply_complex_contraction_vjp( &
            cycle_basis, edge_offsets, all_weights, contraction_bar, values_bar)
        integer, intent(in) :: cycle_basis(:, :), edge_offsets(:)
        real(dp), intent(in) :: all_weights(:)
        complex(dp), intent(in) :: contraction_bar(:, :)
        complex(dp), intent(out) :: values_bar(:, :)
        integer :: edge, cycle_id, component, sample
                ...
                do cycle_id = 1, size(cycle_basis, 2)
                    do component = 1, size(contraction_bar, 1)
                        values_bar(component, sample) = &
                            values_bar(component, sample) + &
                            real(cycle_basis(edge, cycle_id), dp)* &
                            all_weights(sample)* &
                            contraction_bar(component, cycle_id)
                    end do
                end do
            end do
        end do
    end subroutine apply_complex_contraction_vjp

    logical function valid_complex_shape( &
            values, component_count, column_count) result(valid)
        complex(dp), intent(in) :: values(:, :)
        integer, intent(in) :: component_count, column_count

        valid = component_count > 0 .and. size(values, 2) == column_count .and. &
            all(ieee_is_finite(real(values, dp))) .and. &
            all(ieee_is_finite(aimag(values)))
    end function valid_complex_shape

end module fortfem_patch_graph_complex_trace_contraction
