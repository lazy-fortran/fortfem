...
contains
    ...
    subroutine apply_fourier_bilinear_product( &
            input_registry, output_registry, coupling, left_values, right_values, &
            product_values, status)
        !! Apply a bilinear Fourier product with distinct input and output sets.
        !!
        !! The input registry supplies the pair labels and the output registry
        !! supplies the retained projection labels.  A missing output sum is
        !! omitted explicitly; callers can use a padded registry when one
        !! product must be de-aliased.
        type(fourier_mode_registry_t), intent(in) :: input_registry
        type(fourier_mode_registry_t), intent(in) :: output_registry
        ...
        if (.not. validate_bilinear_product_inputs( &
            input_registry, output_registry, coupling, left_values, right_values, &
            product_values, status)) return
        do first_mode = 1, size(input_registry%poloidal_modes)
            do second_mode = 1, size(input_registry%poloidal_modes)
                output_mode = find_fourier_mode(output_registry, &
                    input_registry%poloidal_modes(first_mode) + &
                    input_registry%poloidal_modes(second_mode), &
                    input_registry%toroidal_modes(first_mode) + &
                    input_registry%toroidal_modes(second_mode))
    ...
    end subroutine apply_fourier_bilinear_product

    subroutine apply_fourier_bilinear_product_jvp( &
            input_registry, output_registry, coupling, left_values, right_values, &
            coupling_dot, left_values_dot, right_values_dot, product_values_dot, &
            status)
        !! Apply the product-rule tangent of the projected bilinear product.
        type(fourier_mode_registry_t), intent(in) :: input_registry
        type(fourier_mode_registry_t), intent(in) :: output_registry
        complex(dp), intent(in) :: coupling(:, :, :)
        complex(dp), intent(in) :: left_values(:, :, :), right_values(:, :, :)
        ...
        if (.not. validate_bilinear_product_inputs( &
            input_registry, output_registry, coupling, left_values, right_values, &
            product_values_dot, status)) return
        if (.not. validate_bilinear_product_direction( &
            coupling_dot, left_values_dot, right_values_dot, product_values_dot, &
            size(coupling, 1), size(coupling, 2), size(coupling, 3), &
            size(left_values, 2), size(input_registry%poloidal_modes), &
            size(output_registry%poloidal_modes))) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "Fourier bilinear-product JVP has incompatible increments")
            return
        end if
        do first_mode = 1, size(input_registry%poloidal_modes)
            do second_mode = 1, size(input_registry%poloidal_modes)
        ...
        call status_set(status, FORTSPARSE_OK, "")
    end subroutine apply_fourier_bilinear_product_jvp

    subroutine apply_fourier_bilinear_product_vjp( &
            input_registry, output_registry, coupling, left_values, right_values, &
            product_values_bar, left_values_bar, right_values_bar, coupling_bar, &
            status)
        !! Apply the real adjoint of the projected bilinear product.
        type(fourier_mode_registry_t), intent(in) :: input_registry
        type(fourier_mode_registry_t), intent(in) :: output_registry
        complex(dp), intent(in) :: coupling(:, :, :)
        complex(dp), intent(in) :: left_values(:, :, :), right_values(:, :, :)
        complex(dp), intent(in) :: product_values_bar(:, :, :)
        complex(dp), intent(out) :: left_values_bar(:, :, :)
        complex(dp), intent(out) :: right_values_bar(:, :, :)
        ...
        if (.not. validate_bilinear_product_inputs( &
            input_registry, output_registry, coupling, left_values, right_values, &
            product_values_bar, status)) return
        if (.not. validate_bilinear_product_adjoint( &
            left_values_bar, right_values_bar, coupling_bar, product_values_bar, &
            size(coupling, 1), size(coupling, 2), size(coupling, 3), &
            size(left_values, 2), size(input_registry%poloidal_modes), &
            size(output_registry%poloidal_modes))) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "Fourier bilinear-product VJP has incompatible cotangents")
            return
        end if
        do first_mode = 1, size(input_registry%poloidal_modes)
            do second_mode = 1, size(input_registry%poloidal_modes)
                        ...
                        do left_component = 1, size(left_values, 1)
                            do right_component = 1, size(right_values, 1)
                                ...
                                right_values_bar( &
                                    ...
                                    conjg(coupling(output_component, left_component, &
                                    right_component)*left_values( &
                                    left_component, point, first_mode))*cotangent
                                coupling_bar(output_component, left_component, &
                                    right_component) = &
                                    coupling_bar(output_component, left_component, &
                                    right_component) + conjg(left_values( &
                                    left_component, point, first_mode)*right_values( &
                                    right_component, point, second_mode))*cotangent
                            end do
                        end do
                    end do
        ...
        call status_set(status, FORTSPARSE_OK, "")
    end subroutine apply_fourier_bilinear_product_vjp
    ...
    logical function validate_bilinear_product_inputs( &
            input_registry, output_registry, coupling, left_values, right_values, &
            product_values, status) result(valid)
        type(fourier_mode_registry_t), intent(in) :: input_registry
        type(fourier_mode_registry_t), intent(in) :: output_registry
        complex(dp), intent(in) :: coupling(:, :, :)
        complex(dp), intent(in) :: left_values(:, :, :), right_values(:, :, :)
        ...
        type(fortsparse_status_t), intent(out) :: status
        integer :: output_count, left_count, right_count, point_count
        integer :: input_mode_count, output_mode_count

        valid = .false.
        if (.not. validate_fourier_mode_registry(input_registry, status)) return
        if (.not. validate_fourier_mode_registry(output_registry, status)) return
        output_count = size(coupling, 1)
        left_count = size(coupling, 2)
        right_count = size(coupling, 3)
        point_count = size(left_values, 2)
        input_mode_count = size(input_registry%poloidal_modes)
        ...
        if (output_count < 1 .or. left_count < 1 .or. right_count < 1 .or. &
            ...
            size(left_values, 3) /= input_mode_count .or. &
            size(right_values, 1) /= right_count .or. &
            size(right_values, 2) /= point_count .or. &
            size(right_values, 3) /= input_mode_count .or. &
            size(product_values, 1) /= output_count .or. &
            size(product_values, 2) /= point_count .or. &
            size(product_values, 3) /= output_mode_count) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "Fourier bilinear-product arrays have incompatible shapes")
            return
        end if
        if (.not. finite_complex_3d(coupling) .or. &
            ...
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "Fourier bilinear-product arrays contain non-finite data")
            return
        end if
        valid = .true.
        call status_set(status, FORTSPARSE_OK, "")
    end function validate_bilinear_product_inputs

    logical function validate_bilinear_product_direction( &
            coupling_dot, left_values_dot, right_values_dot, product_values_dot, &
            output_count, left_count, right_count, point_count, input_mode_count, &
            output_mode_count) result(valid)
        complex(dp), intent(in) :: coupling_dot(:, :, :)
        complex(dp), intent(in) :: left_values_dot(:, :, :), right_values_dot(:, :, :)
        complex(dp), intent(in) :: product_values_dot(:, :, :)
        integer, intent(in) :: output_count, left_count, right_count
        integer, intent(in) :: point_count, input_mode_count, output_mode_count
        ...
        valid = size(coupling_dot, 1) == output_count .and. &
            ...
            size(left_values_dot, 3) == input_mode_count .and. &
            size(right_values_dot, 1) == right_count .and. &
            size(right_values_dot, 2) == point_count .and. &
            size(right_values_dot, 3) == input_mode_count .and. &
            size(product_values_dot, 1) == output_count .and. &
            size(product_values_dot, 2) == point_count .and. &
            size(product_values_dot, 3) == output_mode_count .and. &
            finite_complex_3d(coupling_dot) .and. &
            finite_complex_3d(left_values_dot) .and. &
            finite_complex_3d(right_values_dot)
    end function validate_bilinear_product_direction

    logical function validate_bilinear_product_adjoint( &
            left_values_bar, right_values_bar, coupling_bar, product_values_bar, &
            output_count, left_count, right_count, point_count, input_mode_count, &
            output_mode_count) result(valid)
        complex(dp), intent(in) :: left_values_bar(:, :, :)
        complex(dp), intent(in) :: right_values_bar(:, :, :)
        complex(dp), intent(in) :: coupling_bar(:, :, :)
        complex(dp), intent(in) :: product_values_bar(:, :, :)
        integer, intent(in) :: output_count, left_count, right_count
        integer, intent(in) :: point_count, input_mode_count, output_mode_count
        ...
        valid = size(left_values_bar, 1) == left_count .and. &
            ...
            size(left_values_bar, 3) == input_mode_count .and. &
            size(right_values_bar, 1) == right_count .and. &
            size(right_values_bar, 2) == point_count .and. &
            size(right_values_bar, 3) == input_mode_count .and. &
            ...
            size(product_values_bar, 3) == output_mode_count .and. &
            finite_complex_3d(product_values_bar)
    end function validate_bilinear_product_adjoint
