...
contains
    ...
    subroutine assemble_tensor_diffusion_matrix( &
        ...
        call assemble_tensor_diffusion_matrix_impl( &
            basis_gradients, tensor, weights, matrix, 2, status)
    end subroutine assemble_tensor_diffusion_matrix

    subroutine assemble_tensor_diffusion_matrix_nd( &
            basis_gradients, tensor, weights, matrix, status)
        !! Assemble a scalar tensor-diffusion block in any spatial dimension.
        real(dp), intent(in) :: basis_gradients(:, :, :), tensor(:, :, :)
        real(dp), intent(in) :: weights(:)
        real(dp), intent(out) :: matrix(:, :)
        type(fortsparse_status_t), intent(out) :: status
        ...
        call assemble_tensor_diffusion_matrix_impl( &
            basis_gradients, tensor, weights, matrix, 0, status)
    end subroutine assemble_tensor_diffusion_matrix_nd

    subroutine assemble_tensor_diffusion_matrix_3d( &
            basis_gradients, tensor, weights, matrix, status)
        !! Assemble a scalar tensor-diffusion block for three-dimensional cells.
        real(dp), intent(in) :: basis_gradients(:, :, :), tensor(:, :, :)
        real(dp), intent(in) :: weights(:)
        real(dp), intent(out) :: matrix(:, :)
        type(fortsparse_status_t), intent(out) :: status
        ...
        call assemble_tensor_diffusion_matrix_impl( &
            basis_gradients, tensor, weights, matrix, 3, status)
    end subroutine assemble_tensor_diffusion_matrix_3d
    ...
    subroutine assemble_tensor_diffusion_matrix_jvp( &
        ...
        call assemble_tensor_diffusion_matrix_jvp_impl( &
            basis_gradients, tensor, weights, basis_gradients_dot, tensor_dot, &
            weights_dot, matrix_dot, 2, status)
    end subroutine assemble_tensor_diffusion_matrix_jvp

    subroutine assemble_tensor_diffusion_matrix_nd_jvp( &
            basis_gradients, tensor, weights, basis_gradients_dot, tensor_dot, &
            weights_dot, matrix_dot, status)
        real(dp), intent(in) :: basis_gradients(:, :, :), tensor(:, :, :)
        real(dp), intent(in) :: weights(:)
        ...
        call assemble_tensor_diffusion_matrix_jvp_impl( &
            basis_gradients, tensor, weights, basis_gradients_dot, tensor_dot, &
            weights_dot, matrix_dot, 0, status)
    end subroutine assemble_tensor_diffusion_matrix_nd_jvp

    subroutine assemble_tensor_diffusion_matrix_3d_jvp( &
            basis_gradients, tensor, weights, basis_gradients_dot, tensor_dot, &
            weights_dot, matrix_dot, status)
        real(dp), intent(in) :: basis_gradients(:, :, :), tensor(:, :, :)
        real(dp), intent(in) :: weights(:)
        ...
        call assemble_tensor_diffusion_matrix_jvp_impl( &
            basis_gradients, tensor, weights, basis_gradients_dot, tensor_dot, &
            weights_dot, matrix_dot, 3, status)
    end subroutine assemble_tensor_diffusion_matrix_3d_jvp
    ...
    subroutine assemble_tensor_diffusion_matrix_vjp( &
        ...
        call assemble_tensor_diffusion_matrix_vjp_impl( &
            basis_gradients, tensor, weights, matrix_bar, basis_gradients_bar, &
            tensor_bar, weights_bar, 2, status)
    end subroutine assemble_tensor_diffusion_matrix_vjp

    subroutine assemble_tensor_diffusion_matrix_nd_vjp( &
            basis_gradients, tensor, weights, matrix_bar, basis_gradients_bar, &
            tensor_bar, weights_bar, status)
        real(dp), intent(in) :: basis_gradients(:, :, :), tensor(:, :, :)
        real(dp), intent(in) :: weights(:), matrix_bar(:, :)
        ...
        call assemble_tensor_diffusion_matrix_vjp_impl( &
            basis_gradients, tensor, weights, matrix_bar, basis_gradients_bar, &
            tensor_bar, weights_bar, 0, status)
    end subroutine assemble_tensor_diffusion_matrix_nd_vjp

    subroutine assemble_tensor_diffusion_matrix_3d_vjp( &
            basis_gradients, tensor, weights, matrix_bar, basis_gradients_bar, &
            tensor_bar, weights_bar, status)
        real(dp), intent(in) :: basis_gradients(:, :, :), tensor(:, :, :)
        real(dp), intent(in) :: weights(:), matrix_bar(:, :)
        ...
        call assemble_tensor_diffusion_matrix_vjp_impl( &
            basis_gradients, tensor, weights, matrix_bar, basis_gradients_bar, &
            tensor_bar, weights_bar, 3, status)
    end subroutine assemble_tensor_diffusion_matrix_3d_vjp

    subroutine assemble_tensor_diffusion_matrix_impl( &
            basis_gradients, tensor, weights, matrix, expected_dimension, status)
        real(dp), intent(in) :: basis_gradients(:, :, :), tensor(:, :, :)
        real(dp), intent(in) :: weights(:)
        real(dp), intent(out) :: matrix(:, :)
        ...
        if (.not. validate_inputs( &
            basis_gradients, tensor, weights, matrix, expected_dimension, &
            status)) return
        do quadrature = 1, size(weights)
            do first_basis = 1, size(matrix, 1)
                do second_basis = 1, size(matrix, 2)
                    do row = 1, size(basis_gradients, 3)
                        do column = 1, size(basis_gradients, 3)
    ...
    end subroutine assemble_tensor_diffusion_matrix_impl

    subroutine assemble_tensor_diffusion_matrix_jvp_impl( &
            basis_gradients, tensor, weights, basis_gradients_dot, tensor_dot, &
            weights_dot, matrix_dot, expected_dimension, status)
        real(dp), intent(in) :: basis_gradients(:, :, :), tensor(:, :, :)
        real(dp), intent(in) :: weights(:)
        real(dp), intent(in) :: basis_gradients_dot(:, :, :), tensor_dot(:, :, :)
        real(dp), intent(in) :: weights_dot(:)
        ...
        matrix_dot = 0.0_dp
        if (.not. validate_inputs( &
            basis_gradients, tensor, weights, matrix_dot, expected_dimension, &
            status)) return
        if (.not. validate_direction( &
            basis_gradients_dot, tensor_dot, weights_dot, size(weights), &
            size(matrix_dot, 1), size(basis_gradients, 3))) then
        ...
        call status_set(status, FORTSPARSE_OK, "")
    end subroutine assemble_tensor_diffusion_matrix_jvp_impl

    subroutine assemble_tensor_diffusion_matrix_vjp_impl( &
            basis_gradients, tensor, weights, matrix_bar, basis_gradients_bar, &
            tensor_bar, weights_bar, expected_dimension, status)
        real(dp), intent(in) :: basis_gradients(:, :, :), tensor(:, :, :)
        real(dp), intent(in) :: weights(:), matrix_bar(:, :)
        real(dp), intent(out) :: basis_gradients_bar(:, :, :), tensor_bar(:, :, :)
        real(dp), intent(out) :: weights_bar(:)
        ...
        weights_bar = 0.0_dp
        if (.not. validate_inputs( &
            basis_gradients, tensor, weights, matrix_bar, expected_dimension, &
            status)) return
        if (.not. validate_adjoint( &
            basis_gradients_bar, tensor_bar, weights_bar, matrix_bar, &
            size(weights), size(matrix_bar, 1), size(basis_gradients, 3))) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                    ...
                    do row = 1, size(basis_gradients, 3)
                        do column = 1, size(basis_gradients, 3)
                            ...
                            weights_bar(quadrature) = weights_bar(quadrature) + &
                                cotangent*basis_gradients( &
                                quadrature, first_basis, row)*&
        ...
        call status_set(status, FORTSPARSE_OK, "")
    end subroutine assemble_tensor_diffusion_matrix_vjp_impl
    ...
    logical function validate_inputs( &
        ...
        integer, intent(in) :: expected_dimension
        type(fortsparse_status_t), intent(out) :: status
        integer :: quadrature_count, basis_count, dimension_count
        ...
        dimension_count = size(basis_gradients, 3)
        if (quadrature_count < 1 .or. basis_count < 1 .or. &
            dimension_count < 1 .or. (expected_dimension > 0 .and. &
            dimension_count /= expected_dimension) .or. &
            size(tensor, 1) /= quadrature_count .or. &
            size(tensor, 2) /= dimension_count .or. &
            size(tensor, 3) /= dimension_count .or. &
    ...
    logical function validate_direction( &
            ...
            basis_count, dimension_count) result(valid)
        real(dp), intent(in) :: basis_gradients_dot(:, :, :), tensor_dot(:, :, :)
        real(dp), intent(in) :: weights_dot(:)
        integer, intent(in) :: quadrature_count, basis_count, dimension_count

        valid = size(basis_gradients_dot, 1) == quadrature_count .and. &
            size(basis_gradients_dot, 2) == basis_count .and. &
            size(basis_gradients_dot, 3) == dimension_count .and. &
            size(tensor_dot, 1) == quadrature_count .and. &
            size(tensor_dot, 2) == dimension_count .and. &
            size(tensor_dot, 3) == dimension_count .and. &
            size(weights_dot) == quadrature_count .and. basis_count > 0 &
    ...
    logical function validate_adjoint( &
            basis_gradients_bar, tensor_bar, weights_bar, matrix_bar, &
            quadrature_count, basis_count, dimension_count) result(valid)
        ...
        valid = size(basis_gradients_bar, 1) == quadrature_count .and. &
            ...
            size(basis_gradients_bar, 3) == dimension_count .and. &
            size(tensor_bar, 1) == quadrature_count .and. &
            size(tensor_bar, 2) == dimension_count .and. &
            size(tensor_bar, 3) == dimension_count .and. &
            size(weights_bar) == quadrature_count .and. &
            size(matrix_bar, 1) == basis_count .and. &
            size(matrix_bar, 2) == basis_count .and. &
