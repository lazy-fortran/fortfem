module fortfem_fci_interpolation_map
    ...
    !! quadratic-through-quintic stencils. A field-line/map service can use them for a
    ...
    use fortfem_generated_fci_quintic_lagrange, only: &
        generated_fci_quintic_lagrange_weights
    use fortfem_generated_fci_quintic_lagrange_jvp, only: &
        generated_fci_quintic_lagrange_weights_jvp
    use fortfem_generated_fci_quintic_lagrange_vjp, only: &
        generated_fci_quintic_lagrange_weights_vjp
    use fortfem_generated_fci_sextic_lagrange, only: &
        generated_fci_sextic_lagrange_weights
    use fortfem_generated_fci_sextic_lagrange_jvp, only: &
        generated_fci_sextic_lagrange_weights_jvp
    use fortfem_generated_fci_sextic_lagrange_vjp, only: &
        generated_fci_sextic_lagrange_weights_vjp
...
contains
    ...
    subroutine build_fci_quintic_interpolation_map_1d( &
            source_coordinates, target_coordinates, stencil_indices, &
            interpolation_map, status)
        !! Build row-wise quintic Lagrange interpolation weights.
        !!
        !! `stencil_indices(:, row)` supplies six distinct source nodes for
        !! each target.  The local weight kernel is generated by FortSym.
        real(dp), intent(in) :: source_coordinates(:)
        real(dp), intent(in) :: target_coordinates(:)
        integer, intent(in) :: stencil_indices(:, :)
        ...
        integer :: source_count, target_count, row
        integer :: node_index(6), node, other
        real(dp) :: nodes(6), weights(6)

        call status_set(status, FORTSPARSE_INVALID_MATRIX, &
            "FCI quintic interpolation received incompatible coordinates")
        interpolation_map = 0.0_dp
        source_count = size(source_coordinates)
        target_count = size(target_coordinates)
        if (source_count < 6 .or. target_count < 1) return
        if (size(stencil_indices, 1) /= 6 .or. &
            size(stencil_indices, 2) /= target_count) return
        if (size(interpolation_map, 1) /= target_count .or. &
            size(interpolation_map, 2) /= source_count) return
        if (any(.not. ieee_is_finite(source_coordinates)) .or. &
        ...
        do row = 1, target_count
            ...
            if (target_coordinates(row) < minval(nodes) .or. &
                target_coordinates(row) > maxval(nodes)) return
            call generated_fci_quintic_lagrange_weights( &
                target_coordinates(row), nodes(1), nodes(2), nodes(3), &
                nodes(4), nodes(5), nodes(6), weights(1), weights(2), &
        ...
        call status_set(status, FORTSPARSE_OK, "")
    end subroutine build_fci_quintic_interpolation_map_1d

    subroutine build_fci_quintic_interpolation_map_1d_jvp( &
            source_coordinates, target_coordinates, stencil_indices, &
            source_coordinates_dot, target_coordinates_dot, &
            interpolation_map_dot, status)
        !! Apply the fixed-stencil JVP of the quintic map.
        real(dp), intent(in) :: source_coordinates(:)
        real(dp), intent(in) :: target_coordinates(:)
        integer, intent(in) :: stencil_indices(:, :)
        real(dp), intent(in) :: source_coordinates_dot(:)
        ...
        integer :: source_count, target_count, row, node
        integer :: node_index(6)
        real(dp) :: nodes(6), nodes_dot(6), weights_dot(6)
        real(dp), allocatable :: interpolation_map(:, :)

        interpolation_map_dot = 0.0_dp
        source_count = size(source_coordinates)
        target_count = size(target_coordinates)
        if (source_count < 6 .or. target_count < 1) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "FCI quintic JVP received incompatible coordinates")
            return
        end if
        allocate(interpolation_map(target_count, source_count))
        ...
        if (size(source_coordinates_dot) /= source_count .or. &
            ...
            size(interpolation_map_dot, 2) /= source_count) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "FCI quintic JVP received incompatible tangents")
            return
        end if
        if (any(.not. ieee_is_finite(source_coordinates_dot)) .or. &
            any(.not. ieee_is_finite(target_coordinates_dot))) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "FCI quintic JVP received non-finite tangents")
            return
        end if
        ...
        do row = 1, target_count
            ...
            nodes = source_coordinates(node_index)
            nodes_dot = source_coordinates_dot(node_index)
            call generated_fci_quintic_lagrange_weights_jvp( &
                target_coordinates(row), nodes(1), nodes(2), nodes(3), &
                nodes(4), nodes(5), nodes(6), target_coordinates_dot(row), &
                nodes_dot(1), nodes_dot(2), nodes_dot(3), nodes_dot(4), &
                nodes_dot(5), nodes_dot(6), weights_dot(1), weights_dot(2), &
                weights_dot(3), weights_dot(4), weights_dot(5), weights_dot(6))
            do node = 1, 6
                interpolation_map_dot(row, node_index(node)) = weights_dot(node)
            end do
        end do
        call status_set(status, FORTSPARSE_OK, "")
    end subroutine build_fci_quintic_interpolation_map_1d_jvp

    subroutine build_fci_quintic_interpolation_map_1d_vjp( &
            source_coordinates, target_coordinates, stencil_indices, &
            interpolation_map_bar, source_coordinates_bar, &
            target_coordinates_bar, status)
        !! Apply the real VJP of the fixed-stencil quintic map.
        real(dp), intent(in) :: source_coordinates(:)
        real(dp), intent(in) :: target_coordinates(:)
        integer, intent(in) :: stencil_indices(:, :)
        real(dp), intent(in) :: interpolation_map_bar(:, :)
        ...
        integer :: source_count, target_count, row, node
        integer :: node_index(6)
        real(dp) :: nodes(6), weight_bar(6), node_bar(6), target_bar
        real(dp), allocatable :: interpolation_map(:, :)
        ...
        if (source_count < 6 .or. target_count < 1) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "FCI quintic VJP received incompatible coordinates")
            return
        end if
        allocate(interpolation_map(target_count, source_count))
        call build_fci_quintic_interpolation_map_1d( &
        ...
        if (size(interpolation_map_bar, 1) /= target_count .or. &
            ...
            size(target_coordinates_bar) /= target_count) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "FCI quintic VJP received incompatible cotangents")
            return
        end if
        if (any(.not. ieee_is_finite(interpolation_map_bar))) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "FCI quintic VJP received non-finite cotangents")
            return
        end if
        ...
        do row = 1, target_count
            ...
            nodes = source_coordinates(node_index)
            do node = 1, 6
                weight_bar(node) = interpolation_map_bar( &
                    row, node_index(node))
            end do
            call generated_fci_quintic_lagrange_weights_vjp( &
                target_coordinates(row), nodes(1), nodes(2), nodes(3), &
                nodes(4), nodes(5), nodes(6), weight_bar(1), weight_bar(2), &
                weight_bar(3), weight_bar(4), weight_bar(5), weight_bar(6), &
                target_bar, node_bar(1), node_bar(2), node_bar(3), &
                node_bar(4), node_bar(5), node_bar(6))
            target_coordinates_bar(row) = target_coordinates_bar(row) + &
                target_bar
            do node = 1, 6
                source_coordinates_bar(node_index(node)) = &
        ...
        call status_set(status, FORTSPARSE_OK, "")
    end subroutine build_fci_quintic_interpolation_map_1d_vjp

    subroutine build_fci_sextic_interpolation_map_1d( &
            source_coordinates, target_coordinates, stencil_indices, &
            interpolation_map, status)
        !! Build row-wise sextic Lagrange interpolation weights.
        !!
        !! `stencil_indices(:, row)` supplies seven distinct source nodes for
        !! each target.  The local weight kernel is generated by FortSym.
        real(dp), intent(in) :: source_coordinates(:)
        real(dp), intent(in) :: target_coordinates(:)
        integer, intent(in) :: stencil_indices(:, :)
        ...
        integer :: source_count, target_count, row
        integer :: node_index(7), node, other
        real(dp) :: nodes(7), weights(7)

        call status_set(status, FORTSPARSE_INVALID_MATRIX, &
            "FCI sextic interpolation received incompatible coordinates")
        interpolation_map = 0.0_dp
        source_count = size(source_coordinates)
        target_count = size(target_coordinates)
        if (source_count < 7 .or. target_count < 1) return
        if (size(stencil_indices, 1) /= 7 .or. &
            size(stencil_indices, 2) /= target_count) return
        if (size(interpolation_map, 1) /= target_count .or. &
            size(interpolation_map, 2) /= source_count) return
        if (any(.not. ieee_is_finite(source_coordinates)) .or. &
        ...
        do row = 1, target_count
            ...
            if (target_coordinates(row) < minval(nodes) .or. &
                target_coordinates(row) > maxval(nodes)) return
            call generated_fci_sextic_lagrange_weights( &
                target_coordinates(row), nodes(1), nodes(2), nodes(3), &
                nodes(4), nodes(5), nodes(6), nodes(7), weights(1), weights(2), &
        ...
        call status_set(status, FORTSPARSE_OK, "")
    end subroutine build_fci_sextic_interpolation_map_1d

    subroutine build_fci_sextic_interpolation_map_1d_jvp( &
            source_coordinates, target_coordinates, stencil_indices, &
            source_coordinates_dot, target_coordinates_dot, &
            interpolation_map_dot, status)
        !! Apply the fixed-stencil JVP of the sextic map.
        real(dp), intent(in) :: source_coordinates(:)
        real(dp), intent(in) :: target_coordinates(:)
        integer, intent(in) :: stencil_indices(:, :)
        real(dp), intent(in) :: source_coordinates_dot(:)
        ...
        real(dp) :: nodes(7), nodes_dot(7), weights_dot(7)
        real(dp), allocatable :: interpolation_map(:, :)

        interpolation_map_dot = 0.0_dp
        source_count = size(source_coordinates)
        target_count = size(target_coordinates)
        if (source_count < 7 .or. target_count < 1) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "FCI sextic JVP received incompatible coordinates")
            return
        end if
        allocate(interpolation_map(target_count, source_count))
        ...
        if (size(source_coordinates_dot) /= source_count .or. &
            ...
            size(interpolation_map_dot, 2) /= source_count) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "FCI sextic JVP received incompatible tangents")
            return
        end if
        if (any(.not. ieee_is_finite(source_coordinates_dot)) .or. &
            any(.not. ieee_is_finite(target_coordinates_dot))) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "FCI sextic JVP received non-finite tangents")
            return
        end if
        ...
        do row = 1, target_count
            ...
            nodes = source_coordinates(node_index)
            nodes_dot = source_coordinates_dot(node_index)
            call generated_fci_sextic_lagrange_weights_jvp( &
                target_coordinates(row), nodes(1), nodes(2), nodes(3), &
                nodes(4), nodes(5), nodes(6), nodes(7), &
                target_coordinates_dot(row), nodes_dot(1), nodes_dot(2), &
                nodes_dot(3), nodes_dot(4), nodes_dot(5), nodes_dot(6), &
                nodes_dot(7), weights_dot(1), weights_dot(2), weights_dot(3), &
                weights_dot(4), weights_dot(5), weights_dot(6), weights_dot(7))
            do node = 1, 7
                interpolation_map_dot(row, node_index(node)) = weights_dot(node)
            end do
        end do
        call status_set(status, FORTSPARSE_OK, "")
    end subroutine build_fci_sextic_interpolation_map_1d_jvp

    subroutine build_fci_sextic_interpolation_map_1d_vjp( &
            source_coordinates, target_coordinates, stencil_indices, &
            interpolation_map_bar, source_coordinates_bar, &
            target_coordinates_bar, status)
        !! Apply the real VJP of the fixed-stencil sextic map.
        real(dp), intent(in) :: source_coordinates(:)
        real(dp), intent(in) :: target_coordinates(:)
        integer, intent(in) :: stencil_indices(:, :)
        real(dp), intent(in) :: interpolation_map_bar(:, :)
        ...
        integer :: source_count, target_count, row, node
        integer :: node_index(7)
        real(dp) :: nodes(7), weight_bar(7), node_bar(7), target_bar
        real(dp), allocatable :: interpolation_map(:, :)
        ...
        if (source_count < 7 .or. target_count < 1) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "FCI sextic VJP received incompatible coordinates")
            return
        end if
        allocate(interpolation_map(target_count, source_count))
        call build_fci_sextic_interpolation_map_1d( &
        ...
        if (size(interpolation_map_bar, 1) /= target_count .or. &
            ...
            size(target_coordinates_bar) /= target_count) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "FCI sextic VJP received incompatible cotangents")
            return
        end if
        if (any(.not. ieee_is_finite(interpolation_map_bar))) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "FCI sextic VJP received non-finite cotangents")
            return
        end if
        ...
        do row = 1, target_count
            ...
            call generated_fci_sextic_lagrange_weights_vjp( &
                target_coordinates(row), nodes(1), nodes(2), nodes(3), &
                nodes(4), nodes(5), nodes(6), nodes(7), weight_bar(1), &
                weight_bar(2), weight_bar(3), weight_bar(4), weight_bar(5), &
                weight_bar(6), weight_bar(7), target_bar, node_bar(1), &
                node_bar(2), node_bar(3), node_bar(4), node_bar(5), &
                node_bar(6), node_bar(7))
            target_coordinates_bar(row) = target_coordinates_bar(row) + &
                target_bar
            do node = 1, 7
                source_coordinates_bar(node_index(node)) = &
        ...
        call status_set(status, FORTSPARSE_OK, "")
    end subroutine build_fci_sextic_interpolation_map_1d_vjp
