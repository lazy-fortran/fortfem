...
contains
    ...
    subroutine initialize_equilibrium_interchange( &
            ...
            boundary_coordinates, normalization, status)
    ...
    logical function validate_equilibrium_interchange(interchange, status) &
        ...
        if (any(shape(interchange%coefficient_values) /= &
            [interchange%coefficient_count, interchange%sample_count]) .or. &
            size(interchange%coefficient_names) /= interchange%coefficient_count) return
        ...
        if (.not. labels_are_unique_and_nonempty(interchange%coefficient_names) .or. &
            ...
            .not. labels_are_unique_and_nonempty(interchange%boundary_names)) return
        valid = .true.
        status = 0
    ...
    logical function input_shapes_are_compatible( &
        ...
        if (size(coefficients, 1) /= size(coefficient_names) .or. &
            size(coefficients, 2) /= size(mapped, 2)) return
