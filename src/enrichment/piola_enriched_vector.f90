...
contains
    ...
    subroutine evaluate_piola_enriched_vector_differential_2d( &
            map_kind, jacobians, reference_values, reference_curls, &
            reference_divergence, activation, activation_gradient, physical_values, &
            curl_values, divergence, status)
        !! Evaluate affine 2D Piola H(curl)/H(div) enrichment differentials.
        integer, intent(in) :: map_kind
        real(dp), intent(in) :: jacobians(:, :, :), reference_values(:, :, :)
        real(dp), intent(in) :: reference_curls(:, :), reference_divergence(:, :)
        real(dp), intent(in) :: activation(:, :), activation_gradient(:, :, :)
        real(dp), intent(out) :: physical_values(:, :, :)
        real(dp), intent(out) :: curl_values(:, :), divergence(:, :)
        type(fortsparse_status_t), intent(out) :: status
        real(dp) :: mapped(2, size(reference_values, 3))
        real(dp) :: determinant, mapped_curl, mapped_divergence
        integer :: point, basis, local_status

        physical_values = 0.0_dp
        curl_values = 0.0_dp
        divergence = 0.0_dp
        call validate_differential_inputs_2d( &
            map_kind, jacobians, reference_values, reference_curls, reference_divergence, &
            activation, activation_gradient, physical_values, curl_values, divergence, status)
        if (status%code /= FORTSPARSE_OK) return
        do point = 1, size(reference_values, 2)
            ...
            if (local_status /= LINALG_OK) then
                call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                    "Piola differential 2D value map failed")
                return
            end if
            determinant = det2(jacobians(:, :, point))
            do basis = 1, size(reference_values, 3)
                physical_values(:, point, basis) = activation(point, basis)*mapped(:, basis)
                if (map_kind == PIOLA_COVARIANT) then
                    mapped_curl = reference_curls(point, basis)/determinant
                    curl_values(point, basis) = activation(point, basis)*mapped_curl + &
                        activation_gradient(1, point, basis)*mapped(2, basis) - &
                        activation_gradient(2, point, basis)*mapped(1, basis)
                else
                    mapped_divergence = reference_divergence(point, basis)/determinant
                    divergence(point, basis) = activation(point, basis)*mapped_divergence + &
                        dot_product(activation_gradient(:, point, basis), mapped(:, basis))
        ...
        call status_set(status, FORTSPARSE_OK, "")
    end subroutine evaluate_piola_enriched_vector_differential_2d

    subroutine evaluate_piola_enriched_vector_differential_2d_jvp( &
            map_kind, jacobians, reference_values, reference_curls, reference_divergence, &
            activation, activation_gradient, jacobians_dot, reference_values_dot, &
            reference_curls_dot, reference_divergence_dot, activation_dot, &
            activation_gradient_dot, physical_values_dot, curl_values_dot, divergence_dot, &
        ...
        real(dp), intent(in) :: activation_dot(:, :), activation_gradient_dot(:, :, :)
        real(dp), intent(out) :: physical_values_dot(:, :, :)
        real(dp), intent(out) :: curl_values_dot(:, :), divergence_dot(:, :)
        type(fortsparse_status_t), intent(out) :: status
        real(dp) :: mapped(2, size(reference_values, 3))
        real(dp) :: mapped_dot(2, size(reference_values, 3))
        real(dp) :: determinant, determinant_dot, mapped_curl, mapped_curl_dot
        real(dp) :: mapped_divergence, mapped_divergence_dot
        integer :: point, basis, local_status
        ...
        call validate_differential_inputs_2d( &
            ...
            divergence_dot, status)
        if (status%code /= FORTSPARSE_OK) return
        if (.not. validate_differential_direction_2d( &
            jacobians, reference_values, reference_curls, reference_divergence, activation, &
            activation_gradient, jacobians_dot, reference_values_dot, reference_curls_dot, &
            reference_divergence_dot, activation_dot, activation_gradient_dot)) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "Piola differential 2D JVP has incompatible increments")
            return
        end if
        do point = 1, size(reference_values, 2)
            call map_value_jvp_2d( &
            ...
            if (local_status /= LINALG_OK) then
                call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                    "Piola differential 2D JVP value map failed")
                return
            end if
            determinant = det2(jacobians(:, :, point))
            call det2_jvp(jacobians(:, :, point), jacobians_dot(:, :, point), determinant_dot)
            do basis = 1, size(reference_values, 3)
                physical_values_dot(:, point, basis) = activation_dot(point, basis)* &
                    mapped(:, basis) + activation(point, basis)*mapped_dot(:, basis)
                if (map_kind == PIOLA_COVARIANT) then
                    mapped_curl = reference_curls(point, basis)/determinant
                    mapped_curl_dot = reference_curls_dot(point, basis)/determinant - &
                        reference_curls(point, basis)*determinant_dot/determinant**2
                    curl_values_dot(point, basis) = activation_dot(point, basis)*mapped_curl + &
                        activation(point, basis)*mapped_curl_dot + &
                        activation_gradient_dot(1, point, basis)*mapped(2, basis) - &
                        activation_gradient_dot(2, point, basis)*mapped(1, basis) + &
                        activation_gradient(1, point, basis)*mapped_dot(2, basis) - &
                        activation_gradient(2, point, basis)*mapped_dot(1, basis)
                else
                    mapped_divergence = reference_divergence(point, basis)/determinant
                    mapped_divergence_dot = reference_divergence_dot(point, basis)/determinant - &
                        reference_divergence(point, basis)*determinant_dot/determinant**2
        ...
        call status_set(status, FORTSPARSE_OK, "")
    end subroutine evaluate_piola_enriched_vector_differential_2d_jvp

    subroutine evaluate_piola_enriched_vector_differential_2d_vjp( &
            map_kind, jacobians, reference_values, reference_curls, reference_divergence, &
            activation, activation_gradient, physical_values_bar, curl_values_bar, &
            divergence_bar, jacobians_bar, reference_values_bar, reference_curls_bar, &
            reference_divergence_bar, activation_bar, activation_gradient_bar, status)
        ...
        real(dp), intent(in) :: reference_curls(:, :), reference_divergence(:, :)
        real(dp), intent(in) :: activation(:, :), activation_gradient(:, :, :)
        real(dp), intent(in) :: physical_values_bar(:, :, :), curl_values_bar(:, :), divergence_bar(:, :)
        real(dp), intent(out) :: jacobians_bar(:, :, :), reference_values_bar(:, :, :)
        real(dp), intent(out) :: reference_curls_bar(:, :), reference_divergence_bar(:, :)
        real(dp), intent(out) :: activation_bar(:, :), activation_gradient_bar(:, :, :)
        type(fortsparse_status_t), intent(out) :: status
        real(dp) :: mapped(2, size(reference_values, 3))
        real(dp) :: mapped_bar(2, size(reference_values, 3))
        real(dp) :: jacobian_bar_local(2, 2), reference_bar_local(2, size(reference_values, 3))
        real(dp) :: determinant, determinant_bar, determinant_jacobian_bar(2, 2)
        real(dp) :: mapped_curl, mapped_curl_bar, mapped_divergence, mapped_divergence_bar
        integer :: point, basis, local_status
        ...
        call validate_differential_inputs_2d( &
            ...
            divergence_bar, status)
        if (status%code /= FORTSPARSE_OK) return
        if (.not. validate_differential_adjoint_2d( &
            jacobians, reference_values, reference_curls, reference_divergence, activation, &
            activation_gradient, jacobians_bar, reference_values_bar, reference_curls_bar, &
            reference_divergence_bar, activation_bar, activation_gradient_bar)) then
            call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                "Piola differential 2D VJP has incompatible cotangents")
            return
        end if
        do point = 1, size(reference_values, 2)
            call map_value_2d( &
            ...
            if (local_status /= LINALG_OK) then
                call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                    "Piola differential 2D VJP value map failed")
                return
            end if
            determinant = det2(jacobians(:, :, point))
            mapped_bar = 0.0_dp
            ...
            if (map_kind == PIOLA_COVARIANT) then
                do basis = 1, size(reference_values, 3)
                    mapped_curl = reference_curls(point, basis)/determinant
                    mapped_curl_bar = activation(point, basis)*curl_values_bar(point, basis)
                    activation_bar(point, basis) = activation_bar(point, basis) + &
                        curl_values_bar(point, basis)*mapped_curl
                    mapped_bar(1, basis) = mapped_bar(1, basis) - &
                        curl_values_bar(point, basis)*activation_gradient(2, point, basis)
                    mapped_bar(2, basis) = mapped_bar(2, basis) + &
                        curl_values_bar(point, basis)*activation_gradient(1, point, basis)
                    activation_gradient_bar(1, point, basis) = &
                        curl_values_bar(point, basis)*mapped(2, basis)
                    activation_gradient_bar(2, point, basis) = &
                        -curl_values_bar(point, basis)*mapped(1, basis)
                    reference_curls_bar(point, basis) = mapped_curl_bar/determinant
                    determinant_bar = -mapped_curl_bar*reference_curls(point, basis)/determinant**2
                    call det2_vjp(jacobians(:, :, point), determinant_bar, determinant_jacobian_bar)
                    jacobians_bar(:, :, point) = jacobians_bar(:, :, point) + determinant_jacobian_bar
                end do
            else
                do basis = 1, size(reference_values, 3)
            ...
            if (local_status /= LINALG_OK) then
                call status_set(status, FORTSPARSE_INVALID_MATRIX, &
                    "Piola differential 2D VJP value reverse failed")
                return
            end if
            jacobians_bar(:, :, point) = jacobians_bar(:, :, point) + jacobian_bar_local
            reference_values_bar(:, point, :) = reference_bar_local
        ...
        call status_set(status, FORTSPARSE_OK, "")
    end subroutine evaluate_piola_enriched_vector_differential_2d_vjp
    ...
    subroutine validate_differential_inputs_2d( &
            map_kind, jacobians, reference_values, reference_curls, reference_divergence, &
            activation, activation_gradient, values, curl_values, divergence, status)
        integer, intent(in) :: map_kind
        real(dp), intent(in) :: jacobians(:, :, :), reference_values(:, :, :)
        ...
        call status_set(status, FORTSPARSE_INVALID_MATRIX, &
            "Piola differential 2D has incompatible arrays")
        if (map_kind /= PIOLA_COVARIANT .and. map_kind /= PIOLA_CONTRAVARIANT) return
        if (size(reference_values, 1) /= 2) return
        points = size(reference_values, 2)
        basis = size(reference_values, 3)
        if (points < 1 .or. basis < 1) return
        if (size(jacobians, 1) /= 2 .or. size(jacobians, 2) /= 2 .or. &
            size(jacobians, 3) /= points .or. size(reference_curls, 1) /= points .or. &
            size(reference_curls, 2) /= basis .or. size(reference_divergence, 1) /= points .or. &
            size(reference_divergence, 2) /= basis .or. size(activation, 1) /= points .or. &
            size(activation, 2) /= basis .or. size(activation_gradient, 1) /= 2 .or. &
            size(activation_gradient, 2) /= points .or. size(activation_gradient, 3) /= basis .or. &
            size(values, 1) /= 2 .or. size(values, 2) /= points .or. size(values, 3) /= basis .or. &
            size(curl_values, 1) /= points .or. size(curl_values, 2) /= basis .or. &
            size(divergence, 1) /= points .or. size(divergence, 2) /= basis) return
        if (.not. all(ieee_is_finite(jacobians)) .or. &
            .not. all(ieee_is_finite(reference_values)) .or. &
            .not. all(ieee_is_finite(reference_curls)) .or. &
            ...
            .not. all(ieee_is_finite(curl_values)) .or. &
            .not. all(ieee_is_finite(divergence))) return
        do point = 1, points
        ...
        call status_set(status, FORTSPARSE_OK, "")
    end subroutine validate_differential_inputs_2d

    logical function validate_differential_direction_2d( &
            jacobians, reference_values, reference_curls, reference_divergence, activation, &
            activation_gradient, jacobians_dot, reference_values_dot, reference_curls_dot, &
            reference_divergence_dot, activation_dot, activation_gradient_dot) result(valid)
        real(dp), intent(in) :: jacobians(:, :, :), reference_values(:, :, :)
        ...
        valid = size(jacobians_dot, 1) == size(jacobians, 1) .and. &
            ...
            all(ieee_is_finite(reference_curls_dot)) .and. all(ieee_is_finite(reference_divergence_dot)) .and. &
            all(ieee_is_finite(activation_dot)) .and. all(ieee_is_finite(activation_gradient_dot))
    end function validate_differential_direction_2d

    logical function validate_differential_adjoint_2d( &
            jacobians, reference_values, reference_curls, reference_divergence, activation, &
            activation_gradient, jacobians_bar, reference_values_bar, reference_curls_bar, &
            reference_divergence_bar, activation_bar, activation_gradient_bar) result(valid)
        real(dp), intent(in) :: jacobians(:, :, :), reference_values(:, :, :)
        ...
        valid = size(jacobians_bar, 1) == size(jacobians, 1) .and. &
            ...
            size(activation_gradient_bar, 2) == size(activation_gradient, 2) .and. &
            size(activation_gradient_bar, 3) == size(activation_gradient, 3)
    end function validate_differential_adjoint_2d
