...
contains
    ...
    function inner(a, b) result(expr)
        ...
        select type(a)
            ...
            type is (cell_vector_source_t)
            expr_a = create_cell_vector_source(a%values)
            type is (form_expr_t)
            expr_a = a
        class default
        ...
        select type(b)
            ...
            type is (cell_vector_source_t)
            expr_b = create_cell_vector_source(b%values)
            type is (form_expr_t)
            expr_b = b
        class default
    ...
    function cell_coefficient_times_expr(coefficient, expr) result(product)
        type(cell_coefficient_t), intent(in) :: coefficient
        type(form_expr_t), intent(in) :: expr
        type(form_expr_t) :: product
