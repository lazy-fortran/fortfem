...
contains
    ...
    function mesh_from_domain_triangle(vertices, segments, hole_points, &
        ...
        call triangulate_compat(vertices, segments, holes, result, stat, &
            min_angle=min_angle, quality=min_angle > 0.0_dp, nobisect=1)
        if (stat /= 0 .or. result%ntriangles == 0) then
            mesh%data%n_vertices = 0
            mesh%data%n_triangles = 0
            return
        end if
