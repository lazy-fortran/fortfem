...
#### API migration status on `main` (2026-08-03)
...
- **API-05 — first migration slices complete:** nineteen representative examples
  ...
  passes.
  The basic FEM clients then moved off the transitional narrow
  `fortfem_api_mesh/forms/spaces/solvers` modules in `e4aefd0`; a new
  `test_basic_fem_facade` behavioral smoke covers the mesh/space contract.
  The only remaining umbrella import is the intentional tetrahedral Poisson
  compatibility smoke.
...
| GALLERY-BIRO-TEAM | **neutral ladder complete**: `biro_tree_cotree_benchmark` and the 3-D `biro_tree_cotree_3d_gallery` provide provenance-pinned tree--cotree curl--curl reductions, direct reconstruction, face-circulation/energy/residual CSV, and solution-first graph/cube vector plots; `team3_neutral_benchmark`, `team7_neutral_benchmark`, `team13_neutral_benchmark`, and `team20_neutral_benchmark` provide license-safe TEAM-shaped manufactured fields, probe/energy diagnostics, explicit provenance JSON, and solution-first magnitude/vector/geometry plots. Exact Biro/TEAM arrays, readers, nonlinear material laws, and reference curves remain external-data work and do not enter FortFEM. | Independent tree/cotree restriction/prolongation and reduced-system oracles; finite TEAM field/probe/energy manifests; generated media remain ignored and no paper/TEAM source or reader is copied |
| GALLERY-BIRO-EXACT-DATA | Keep the method-faithful Bíró tree--cotree fixture in FortFEM, and use the metadata-only `benchmark/external_oracles/biro_paper_manifest.json` plus `tools/validate_biro_external_manifest.py` adapter slot for the paper’s exact geometry/material/source arrays when the user supplies a redistributable or sister-repository dataset. The new `run_biro_paper_gallery.py` validates the DOI, payload schema, URI/checksum/path and writes a solution-first SVG/CSV/provenance record; it explicitly skips absent data and never labels the manufactured cube as an exact paper figure or copies copyrighted arrays/source. | With an approved external manifest, the renderer can produce a paper-data solution preview and records for a downstream FEM solver; without it, the neutral fixture remains the only checked-in oracle and the exact gallery reports its limitation explicitly |
| GALLERY-TEAM-LADDER | Extend the neutral TEAM gallery one problem at a time (TEAM-3/7/13/20) through external manifests and optional sister-data jobs. The `team3_neutral_benchmark`, `team7_neutral_benchmark`, `team13_neutral_benchmark`, and `team20_neutral_benchmark` are now manufactured, license-safe fixtures with solution-first 2-D/3-D vector/surface plots, probe/energy CSV, explicit provenance JSON, and bounded physical-first shell oracles. Each in-repo fixture remains tiny, analytic, and clearly marked as non-exact; exact probes, B-H curves, and reference measurements stay outside FortFEM. | Every enabled problem has a checksum/license/revision manifest, independent field/probe/energy oracle, mesh-complete solution plot, and a bounded optional external runner; absent data is a documented skip, not a fabricated comparison |

| API-05-SOLUTION-FIRST | Migrate the remaining gallery in scalar-to-vector-to-interface order. For each example, assign the smallest canonical facade, make the first generated panel a physical solution (scalar field, vector arrows, surface, or field line), and retain convergence/diagnostic panels after it. Add a bounded shell gate that checks the primary plot and an independent CSV/JSON oracle without committing media. `circular_dtn_modes` now records its physical mode map before diagnostics and has a ten-second PNG/stage-order/discarded-mode oracle; `laplace_exterior_bem_circle` now emits solution samples/provenance and independently verifies the exterior (u=x/r^2) field before diagnostics; `maxwell_vector_open_boundary_parity_gallery` now leads with a nonzero toroidal FEM--BEM vector slice and PML vector field, then records DtN/larger-domain diagnostics with a fast gate. | The example-doc coverage test reports every executable example; each migrated example runs under the ten-second gate, produces a non-empty primary data record, and its independent manufactured/analytical oracle passes |
| API-07-CONSUMER-MATRIX | Add disjoint downstream-style smoke clients for the growing product boundaries: core geometry/mesh, FEEC/IGA, Fourier/special functions, FEM--BEM/DtN/PML, mixed waves/elasticity, solver/gauge, time integration, plotting, and interop. Each client imports only its canonical facade and records the public symbols it intentionally uses. The wave/response slice migrates complex low-rank response, wall-response condensation, and wave-reflection diagnostics tests to `fortfem_interop`/`fortfem_boundary`; the force/tree slice migrates force residual/objective derivatives and sparse tree--cotree direct solves to `fortfem_feec`; the curl--curl client moves Nédélec solution evaluation to `fortfem_feec` plus affine inversion to `fortfem_core`, while the toroidal-harmonic/Ampere clients move to `fortfem_fourier` (`4a11fff`, `a0547d1`); the boundary slices move trace metadata, circular DtN, Helmholtz DtN, and scalar/curl--curl Cartesian/curvilinear PML consumers to `fortfem_boundary` (`69881fc`, `472cea4`); the axis-regular Fourier/radial clients move existing mode-table and radial value/JVP/VJP contracts to `fortfem_fourier`/`fortfem_core` (`81134c1`); the vector plotting client separates core mesh/space, FEEC vector forms, and `fortfem_plot` output (`594928f`); the acoustic clients move planar DtN and curved displacement-form tests to `fortfem_boundary` (`bd99c83`); the time/solver clients move dissipative Cayley to `fortfem_time` and CG derivatives to `fortfem_advanced_solvers` (`5fe7283`); the advanced-solver client moves its mesh/space/assembly usage to `fortfem_core`/`fortfem_feec` (`f3d01c5`); the block-graph residual/JVP/VJP client moves to `fortfem_feec` (`e729f83`); the next batches migrate six Fourier tests to `fortfem_fourier` (`1f8cbef`), five Helmholtz BEM/CFIE tests to `fortfem_boundary` (`e451bc6`), six FCI/field-line/anisotropic tests to `fortfem_feec` (`0fcbf6f`), six BEM representation tests to `fortfem_boundary` plus torus mesh ownership to `fortfem_core` (`e939b02`), six IGA/B-spline FEEC tests to `fortfem_feec` (`91c4124`), six DG/HDG sequence consumers to `fortfem_feec` (`f7e1b30`), six complex/force residual consumers to `fortfem_feec`/`fortfem_boundary` (`b69d80a`), and six cut/enrichment sequence consumers to `fortfem_feec` (`2d54659`). All are existing-symbol migrations with no duplicate implementation. | Each client compiles without `fortfem_api`, reproduces an independent value/JVP/VJP or structure-ledger result, and the inventory has one owner plus one migration note for every imported symbol |
...
The API-05/API-07 facade queue has been handed off and its worktrees removed.
The current working rule remains: create disjoint branches under
`/mnt/storage/code/lazy-fortran/worktrees/`, rebase from current `main`, write
an independent behavioral test before implementation, avoid ROADMAP and
...
`tools/validate_external_oracle_runner.py` define the metadata-only path for
FreeFEM, MFEM, and FEniCSx. The contract records executable/source revisions,
licenses, checksums, common sampling, tolerances, timing, peak memory,
runner identity, optional `uv` post-processing, sister-repository data, and a
mandatory skip reason when a heavy code is unavailable. Its shell test uses a
small analytical Poisson manifest and an independently invalidated skip case;
no external source or benchmark data is copied into FortFEM.
...
The next API-07 queue is deliberately limited to disjoint, representative
consumers: remaining IGA/Fourier and tensor-pressure examples, boundary/PML
clients, and plotting/time-integration clients. Each hand-off must identify
the exact legacy imports removed, use an existing canonical owner where
possible, add a behavioral oracle before changing imports, and leave the
umbrella import only in the documented compatibility smoke. No broad rename,
physics implementation, EQDSK reader, or external-code source belongs in this
foundation repository.

The latest completed consumer batches extend this audit without changing
numerical ownership: complex/force residuals (`b69d80a`), cut/enrichment FEEC
spaces (`2d54659`), and Maxwell/open-boundary consumers
...
The solution-performance slice `da4d9835` adds bounded timing and accuracy
metadata to the solution-first mixed Poisson gallery and the slab, cylinder,
sphere, and torus Ampere sheet-current gallery. Its independent six-second
gate recomputes the integrated-current error from the emitted ledger and checks
positive timing, mesh-size, and solution-order fields. Generated media remain
ignored.
...
The scalar/core slice `58a4b5e` and the Fourier registry/toroidal slice
`e98cac1` are now also integrated. They migrate six scalar assembly/solver
consumers and six Fourier/special-function consumers, respectively, while
keeping vector boundary-condition ownership in FEEC (`fc31629`) and harmonic
normalization in `fortfem_fourier`. Their analytical, assembly, solver,
modal, geometry, and derivative tests pass serially. The full `fo` checks
...
| --- | --- | --- |
| Mesh generation | Native triangular and quadrilateral mesh paths plus a dependency-free Triangle-compatible constrained Delaunay, hole-carving, and `pqY` quality-refinement library are on `main`; the compatibility path is available through the Fortran API, CLI, and standalone CMake target | Restore the Triangle golden-mesh parity test on the current integrated source tree and verify the optional live Triangle oracle in CI |
...
| FEEC | Oriented triangular and tetrahedral H1, H(curl), H(div), and DG families, Piola maps, commuting tests, sparse assembly, mixed RT-DG Poisson, neutral real and complex two-field/packed N-field block graph residuals with complete JVP/VJP contracts, real/complex packed-graph CSC adapters, retained real/complex field-split solve JVP/VJP products, retained coupled Schur value/JVP/VJP composition, and complex anisotropic tensor-power value/JVP/VJP contracts | FortSym manufactured composed forms, energy/power ledgers, and arbitrary multipatch assembly |
...
- The `sheet_current_surface_gallery` extends that physical-first view to
  oriented slab, cylinder, sphere, and torus surfaces. It draws the
  manufactured tangential \(\mathbf K\) vectors on each surface, records the
  fitted/resolved integrated ledger and fixed-topology quadrature data, and
  runs an independent bounded gate before diagnostics. It remains a neutral
  surface representation fixture; geometry and PDE closures are caller-owned.
