...
| PML | Scalar and curl-curl Cartesian complex-stretching tensors with slab, triangular, and tetrahedral examples, plus full complex 3-by-3 curvilinear scalar and curl-curl coefficient maps and tetrahedral Nédélec element JVP/VJP with diagonal-reduction oracles | Automated curved-object layers, reflection/error metrics, and derivative coverage for all geometry parameters |
...
- The tetrahedral first-kind Nédélec local PML form now consumes those full
  ...
  the contract. Global curvilinear PML CSC assembly and solved-state products
