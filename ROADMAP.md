...
| Fourier and toroidal modes | The fixed-topology mode registry now provides field-period phase, normalization, conjugate packing, retained-triad lookup, an ordered triad map with omitted-sum count, a deterministic one-product padded work registry, radial regularity, complex coordinate derivatives, a same-registry three-factor vector/tensor Fourier product, and a distinct-registry bilinear application with JVP/VJP; FortNum now exposes independently tested ordinary Legendre Q values/derivatives, orthonormal complex spherical harmonics with angular derivatives and Gaunt products, and Hobson-normalized half-integer toroidal P/Q values/derivatives; model-specific mode operators and branch data remain | Symmetry and mode-by-mode energy checks, repeated-product work-set policies, and nonlinear application composition |
