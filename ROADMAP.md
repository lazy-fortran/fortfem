...
exact-sequence oracles. The currently active unique worktrees are
`migrate_tensor_wave_batch`, `migrate_scalar_core_batch`, and
`migrate_boundary_small_batch`; each is limited to its listed files and must
be integrated only after its focused behavioral tests pass. The inventory is
