...
| Linear response interchange | Neutral modal `(m,n)` metadata, provenance, complex equilibrium/inertia/resistive/vacuum/wall blocks, response channels, the (K-omega^2M+iomega R+V+W) operator and forced residual with complex JVP/VJP products, reciprocity/passivity diagnostics, and a common weighted physical sample-set contract are public and independently tested | FEM/BEM/DtN/PML assembly-specific response matrices, singular-layer matching, and external-code sampler fixtures |
