...
The 2026-08-03 dispatches handed off clean branches. The earlier component
mortar, torus-vector, sheet-current, TEAM provenance, exterior-BEM,
solution-first, wave/response, force/tree, curl--curl, toroidal-harmonic,
boundary-DtN/PML, axis-regular, plotting, acoustic, time/solver,
advanced-solver, block-graph, Fourier, BEM/CFIE, FCI, representation-BEM,
IGA/B-spline, and DG/HDG slices remain recorded in their commits above and
preserve their independent physical, derivative, geometry, modal, and
exact-sequence oracles. The latest unique worktrees
`migrate_tensor_wave_batch`, `migrate_scalar_core_batch`,
`migrate_boundary_small_batch`, and `migrate_fourier_registry_batch` have all
been integrated after their focused tests and removed. The inventory is
regenerated by the integrator after every verified slice. Future agents must
take the next disjoint example batch rather than reopening these files or
duplicating their exports.
