...
| MHD-02 | **active** | Finish the compatible spatial core: H1--H(curl)--H(div)--L2, Fourier-FEM, IGA patch graphs, orientation/periodic maps, harmonic forms, and tree--cotree direct gauge reduction for curl--curl systems. The topology-only `build_multipatch_signed_dof_map` now constructs arbitrary-patch signed maps and rejects inconsistent orientation cycles; geometry-aware trace maps and distributed ownership are still open | Slab, cylinder, sphere, and torus complexes satisfy incidence identities, divergence/curl defects, loop periods, gauge reduction, and direct-solve parity; fixed selectors are not differentiated |
