...
The 2026-08-03 dispatches handed off clean branches; the wave/tensor branch
was verified as byte-identical to the already integrated `03b511b` tree, so it
was deliberately not cherry-picked. The integrator then added the component
mortar, torus-vector, four-geometry sheet-current, TEAM provenance,
exterior-BEM, and solution-first documentation slices, followed by the
wave/response test migration in `b605b61` and the force/tree consumer
migration in `95a0df5`; the follow-up curl--curl, toroidal-harmonic, and
boundary-DtN/PML and axis-regular Fourier/radial consumer slices landed in
`4a11fff`, `a0547d1`, `69881fc`, `81134c1`, `472cea4`, `594928f`,
`bd99c83`, `5fe7283`, `f3d01c5`, and `e729f83`. They move the remaining
representative curl--curl solution client to
`fortfem_feec` (with affine inversion owned by `fortfem_core`), the toroidal
harmonic/Ampere clients to `fortfem_fourier`, and trace metadata/circular DtN
clients to `fortfem_boundary`, and axis-regular mode/radial clients to
`fortfem_fourier`/`fortfem_core`, with no implementation or duplicate exports.
The vector plotting client additionally records the core/FEEC/plot facade
ownership split without changing plotting behavior; the acoustic and
time/solver clients likewise preserve their independent physical and
derivative oracles; the advanced-solver and block-graph clients preserve
their sparse and generated-kernel checks. The Fourier, BEM/CFIE, FCI,
representation-BEM, IGA/B-spline, and DG/HDG batches likewise preserve
modal, boundary, flux, field-line, geometry, and exact-sequence oracles
(`1f8cbef`, `e451bc6`, `0fcbf6f`, `e939b02`, `91c4124`, `f7e1b30`).
The inventory is current after
  `tools/generate_api_public_inventory.py`. Future agents must take the next
disjoint example batch rather than reopening these files or duplicating their
exports.
