...
| Sparse algebra | FortSparse CSC assembly, retained factors, real and complex solves, sparse products, tree--cotree CSC direct reductions with fixed-map JVP/VJP, and CG, PCG, GMRES, and BiCGSTAB converged-state derivative contracts; dense and standalone sparse IC(0)/ILU(0) factor/apply paths plus a deterministic sparse fixed-factor ILUT path are public | Scalable row-oriented ILUT construction, fill-controlled ICHOL, PCG integration and measured scaling, flexible Krylov products, and block solver derivatives |
...
scalable row-oriented construction and fill-controlled ICHOL remain. The
