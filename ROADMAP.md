...
wave/response test migration in `b605b61` and the force/tree consumer
  migration in `95a0df5`; the follow-up curl--curl and toroidal-harmonic
  consumer slices landed in `4a11fff` and `a0547d1`, respectively. They move
  the remaining representative curl--curl solution client to `fortfem_feec`
  (with affine inversion owned by `fortfem_core`) and the toroidal harmonic /
  Ampere clients to `fortfem_fourier`, with no implementation or duplicate
  exports. The inventory is current after
