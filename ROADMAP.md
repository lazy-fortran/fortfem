...
| Cut FEEC spaces | The scalar shifted-Heaviside activation and matrix-level shifted enriched-space constructor, a 3D vector-enrichment curl/divergence product-rule diagnostic, the physical vector/metric support Gram contraction, batched 2D/3D covariant/contravariant Piola-enrichment composition, and a rectangular commuting-projection audit with value/JVP/VJP actions are public; Piola-aware vector-compatible XFEM/XIGA and DG spaces that preserve or explicitly report the de Rham sequence across cuts remain | Curl-gradient and divergence-curl identities on every generated space, vector/XIGA constructors, and fitted versus unfitted convergence |
