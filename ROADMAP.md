...
| Open boundaries | Planar, circular, and spherical scalar Helmholtz DtN paths, scalar BEM, Maxwell trace and PML components, a mixed RWG/RBC weak Maxwell DtN map assembled from exact-curved torus EFIE/MFIE/mass forms, and toroidal Laplace off-surface reconstruction with an optional target gradient and fundamental-solution oracle | Curved-map JVP/VJP geometry products, larger-domain toroidal FEM/BEM/DtN/PML comparisons, and robust vector field reconstruction fixtures |
...
- The public `fortfem_maxwell_curved_dtn` contract now composes weak electric
  ...
  remain active.
- `evaluate_laplace_representation_torus_curved_3d` now optionally returns the
  reconstructed target gradient.  An independent curved-torus test samples a
  fundamental solution and its normal trace on two meshes, checks field
  convergence and the off-surface gradient, and leaves the larger-domain and
  geometry-derivative comparisons as explicit follow-up work.
