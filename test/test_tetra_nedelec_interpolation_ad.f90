program test_tetra_nedelec_interpolation_ad
    ...
    call initialize_tetra_nedelec_first_kind(3, basis, status)
    call tetra_nedelec_interpolation_points(basis, vertices, points, status)
    ...
    call check(maxval(abs(dofs - dofs_callback)) < 2.0e-11_dp, &
        "sampled Nedelec interpolation matches callback")
    ...
    call check(maxval(abs(dofs_dot - &
        (dofs_plus - dofs_minus)/(2.0_dp*step))) < 8.0e-8_dp, &
        "sampled Nedelec interpolation JVP")
    ...
    call check(abs(lhs - rhs) < 5.0e-11_dp*max(1.0_dp, abs(lhs), abs(rhs)), &
        "sampled Nedelec interpolation adjoint identity")
...
contains
    ...
    subroutine initialize_sample_data( &
        ...
        allocate(field_gradients%edge(3, 3, size(sample_points%edge, 2), 6))
        ...
        call fill_values(sample_points%edge, field_samples%edge, &
            field_gradients%edge, tangent_samples%edge)
    ...
    subroutine evaluate_samples( &
        ...
        field_samples%edge = field_samples%edge + scale*tangent_samples%edge
    ...
    pure real(dp) function sample_dot(first, second) result(value)
        ...
        value = sum(first%edge*second%edge) + sum(first%face*second%face) + &
...
end program test_tetra_nedelec_interpolation_ad
