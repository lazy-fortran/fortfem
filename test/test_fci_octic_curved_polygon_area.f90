program test_fci_octic_curved_polygon_area
    ...
    real(dp) :: controls(2, 7, vertex_count, cell_count)
    real(dp) :: straight_controls(2, 7, vertex_count, cell_count)
    real(dp) :: bad_vertices(2, vertex_count, cell_count)
    real(dp) :: bad_controls(2, 7, vertex_count, cell_count)
    real(dp) :: vertices_dot(2, vertex_count, cell_count)
    real(dp) :: controls_dot(2, 7, vertex_count, cell_count)
    real(dp) :: vertices_bar(2, vertex_count, cell_count)
    real(dp) :: controls_bar(2, 7, vertex_count, cell_count)
    real(dp) :: vertices_plus(2, vertex_count, cell_count)
    real(dp) :: vertices_minus(2, vertex_count, cell_count)
    real(dp) :: controls_plus(2, 7, vertex_count, cell_count)
    real(dp) :: controls_minus(2, 7, vertex_count, cell_count)
    ...
    call compute_fci_octic_curved_polygon_cell_areas_2d( &
        vertices, controls, areas, status)
    call record_condition(status%code == 0, &
        "FCI octic curved polygon area accepts curved cells")
    do cell = 1, cell_count
    ...
    call record_condition(maxval(abs(areas - expected)) < 8.0e-12_dp, &
        "FCI octic curved polygon area matches 11-point Gauss-Green oracle")
    call record_condition(maxval(abs(areas - straight)) > 1.0e-3_dp, &
        "FCI octic curved polygon area resolves edge curvature")
    call record_condition(all(areas > 0.0_dp), &
        "FCI octic curved polygon areas are positive")

    call compute_fci_octic_curved_polygon_cell_areas_2d_jvp( &
    ...
    call record_condition(maxval(abs(areas_dot - &
        ...
        "FCI octic curved polygon area JVP matches central differences")

    call compute_fci_octic_curved_polygon_cell_areas_2d_vjp( &
    ...
    call record_condition(abs(lhs - rhs) < &
        ...
        "FCI octic curved polygon area VJP satisfies the real dot-product identity")

    call compute_fci_octic_curved_polygon_cell_areas_2d( &
        vertices, straight_controls, areas_plus, status)
    call record_condition(maxval(abs(areas_plus - straight)) < 8.0e-12_dp, &
        "FCI octic curved area recovers straight polygon area")
    ...
    call record_condition(status%code /= 0 .and. all(areas_plus == 0.0_dp), &
        "FCI octic curved polygon area rejects degenerate cells")

    call check_summary("FCI octic curved polygon area")
...
contains
    ...
    pure real(dp) function gauss_green_area(cell_vertices, edge_controls) &
            result(area)
        real(dp), intent(in) :: cell_vertices(2, vertex_count)
        real(dp), intent(in) :: edge_controls(2, 7, vertex_count)
        ...
        do edge = 1, vertex_count
            ...
            do quadrature = 1, size(gauss_points)
                call octic_bezier_point_and_tangent( &
    ...
    pure subroutine octic_bezier_point_and_tangent( &
            first, controls, last, parameter, point, tangent)
        real(dp), intent(in) :: first(2), controls(2, 7), last(2), parameter
        real(dp), intent(out) :: point(2), tangent(2)
        real(dp) :: bernstein(0:8), derivative_bernstein(0:7)
        real(dp) :: control_points(2, 0:8), one_minus_parameter
        integer :: index

        one_minus_parameter = 1.0_dp - parameter
        control_points(:, 0) = first
        control_points(:, 1:7) = controls
        control_points(:, 8) = last
        bernstein = [ &
        ...
        do index = 0, 7
            tangent = tangent + 8.0_dp*derivative_bernstein(index)* &
                (control_points(:, index + 1) - control_points(:, index))
        end do
    end subroutine octic_bezier_point_and_tangent
...
end program test_fci_octic_curved_polygon_area
