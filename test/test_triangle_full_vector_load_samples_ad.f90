program test_triangle_full_vector_load_samples_ad
...
contains
    ...
    subroutine check_family(normal_family, label)
        ...
        if (normal_family) then
            ...
            call assemble_triangle_bdm_vector_load_samples( &
                plus_mesh, degree, quadrature_degree, source_plus, load_plus, &
                status)
            call assemble_triangle_bdm_vector_load_samples( &
                minus_mesh, degree, quadrature_degree, source_minus, &
                load_minus, status)
        else
        ...
        call check(status%code == 0, label//" sampled load products succeed")
        call check(maxval(abs( &
            load_dot - (load_plus - load_minus)/(2.0_dp*step))) < 4.0e-9_dp, &
            label//" sampled load JVP matches full Eulerian reassembly")
        if (allocated(load_bar)) deallocate(load_bar)
        ...
        if (normal_family) then
            call assemble_triangle_bdm_vector_load_samples_vjp( &
                mesh, degree, quadrature_degree, source, source_gradients, &
                load_bar, vertices_bar, source_bar, status)
        else
        ...
        call check(status%code == 0, label//" sampled load VJP succeeds")
        call check( &
            abs(lhs - rhs) < 4.0e-11_dp*max(1.0_dp, abs(lhs), abs(rhs)), &
            label//" sampled load products obey the adjoint identity")
...
end program test_triangle_full_vector_load_samples_ad
