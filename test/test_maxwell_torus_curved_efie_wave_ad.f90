program test_maxwell_torus_curved_efie_wave_ad
    ...
    call assemble_maxwell_torus_efie_wave_number_jvp( &
        ...
        wave_number, impedance, 2, 1.0e-5_dp, 1, wave_number_dot, matrix, &
        matrix_dot, status)
    call assemble_maxwell_torus_curved_efie_rwg_3d( &
        vertices, triangles, parameters, major_radius, minor_radius, &
        wave_number + step*wave_number_dot, impedance, 2, 1.0e-5_dp, 1, &
    ...
    call record_condition(status == 0, &
        "toroidal propagating EFIE wave-number JVP succeeds")
    ...
    call record_condition(jvp_error < 2.0e-6_dp, &
        "toroidal propagating EFIE wave-number JVP matches reassembly")
    ...
    call assemble_maxwell_torus_efie_wave_number_vjp( &
        vertices, triangles, parameters, major_radius, minor_radius, &
        wave_number, impedance, 2, 1.0e-5_dp, 1, matrix_bar, wave_number_bar, &
        status)
    lhs = real(sum(conjg(matrix_bar)*matrix_dot), dp)
    rhs = wave_number_bar*wave_number_dot
    call record_condition(status == 0, &
        "toroidal propagating EFIE wave-number VJP succeeds")
    call record_condition(abs(lhs - rhs) < &
        2.0e-7_dp*max(1.0_dp, abs(lhs), abs(rhs)), &
        "toroidal propagating EFIE wave-number products obey the adjoint identity")

    call check_summary("Differentiable toroidal propagating EFIE")
...
end program test_maxwell_torus_curved_efie_wave_ad
