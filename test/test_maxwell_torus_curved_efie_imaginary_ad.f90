program test_maxwell_torus_curved_efie_imaginary_ad
    ...
    call assemble_maxwell_torus_efie_imaginary_impedance_jvp( &
        vertices, triangles, parameters, major_radius, minor_radius, &
        decay_rate, impedance, 2, 1.0e-5_dp, 1, impedance_dot, matrix, &
        matrix_dot, status)
    call assemble_maxwell_torus_curved_efie_imaginary_rwg_3d( &
        vertices, triangles, parameters, major_radius, minor_radius, &
        decay_rate, impedance + step*impedance_dot, 2, &
        1.0e-5_dp, 1, matrix_plus, status_plus)
    call assemble_maxwell_torus_curved_efie_imaginary_rwg_3d( &
        vertices, triangles, parameters, major_radius, minor_radius, &
        decay_rate, impedance - step*impedance_dot, 2, &
        1.0e-5_dp, 1, matrix_minus, status_minus)
    jvp_error = maxval(abs(matrix_dot - &
        (matrix_plus - matrix_minus)/(2.0_dp*step)))
    call record_condition(status == 0, &
        "toroidal imaginary EFIE parameter JVP succeeds")
    call record_condition(status_plus == 0 .and. status_minus == 0, &
        "perturbed toroidal imaginary EFIE assembles")
    call record_condition(jvp_error < 2.0e-8_dp, &
        "toroidal imaginary EFIE parameter JVP matches reassembly")
    ...
    call assemble_maxwell_torus_efie_imaginary_impedance_vjp( &
        vertices, triangles, parameters, major_radius, minor_radius, &
        decay_rate, impedance, 2, 1.0e-5_dp, 1, matrix_bar, impedance_bar, &
        status)
    lhs = real(sum(conjg(matrix_bar)*matrix_dot), dp)
    rhs = impedance_bar*impedance_dot
    call record_condition(status == 0, &
        "toroidal imaginary EFIE parameter VJP succeeds")
    call record_condition(abs(lhs - rhs) < &
        2.0e-9_dp*max(1.0_dp, abs(lhs), abs(rhs)), &
        "toroidal imaginary EFIE parameter products obey the adjoint identity")

    call check_summary("Differentiable toroidal imaginary-wavenumber EFIE")
...
end program test_maxwell_torus_curved_efie_imaginary_ad
