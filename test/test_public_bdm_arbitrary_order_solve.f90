program test_public_bdm_arbitrary_order_solve
    ...
    do degree = 1, 4
        ...
        call check_condition(maximum_error < 2.0e-10_dp, &
            "Public arbitrary-order BDM solve reproduces a constant field")
    end do
    call check_summary("Public arbitrary-order BDM solve")
...
contains
    ...
    subroutine solve_constant_field(degree, error)
        ...
        space = vector_function_space(mesh, "BDM", degree)
        expected_dof_count = (degree + 1) * mesh%data%n_edges + &
            (degree * degree - 1) * mesh%data%n_triangles
        if (space%ndof /= expected_dof_count) then
            error stop "public arbitrary-order BDM space has wrong dimension"
        end if
    ...
    subroutine measure_constant_field_error( &
        ...
        call build_triangle_full_vector_dof_map( &
            public_mesh%data, degree, global_dofs, transforms, &
            global_count, status)
        if (status /= 0) error stop "arbitrary-order BDM global map failed"
        call initialize_triangle_bdm(degree, basis, status)
        if (status /= 0) error stop "arbitrary-order BDM basis failed"
        ...
        do triangle = 1, public_mesh%data%n_triangles
            ...
            if (status /= 0) error stop "BDM field evaluation failed"
...
end program test_public_bdm_arbitrary_order_solve
