program test_triangle_rt_manufactured_convergence
    ...
    real(dp) :: coarse_divergence_error, coarse_l2_error
    real(dp) :: divergence_rate, fine_divergence_error, fine_l2_error, l2_rate
    ...
    do degree = 0, 3
        order = degree + 1
        call interpolation_errors( &
            3, degree, coarse_l2_error, coarse_divergence_error)
        call interpolation_errors( &
            5, degree, fine_l2_error, fine_divergence_error)
        l2_rate = log(coarse_l2_error / fine_l2_error) / log(2.0_dp)
        divergence_rate = log( &
            coarse_divergence_error / fine_divergence_error) / log(2.0_dp)
        call record_condition(fine_l2_error < coarse_l2_error .and. &
            fine_divergence_error < coarse_divergence_error, &
            "Manufactured H(div) interpolation improves under refinement")
        call record_condition(l2_rate > real(order, dp) - 0.35_dp, &
            "Manufactured flux-field error reaches the expected h-order")
        call record_condition( &
            divergence_rate > real(order, dp) - 0.45_dp, &
            "Manufactured divergence error reaches the expected h-order")
    end do

    call check_summary("Arbitrary-order RT manufactured convergence")
    if (.not. all_passed) error stop 1

contains

    subroutine interpolation_errors( &
            vertex_count, degree, l2_error, divergence_error)
        integer, intent(in) :: vertex_count, degree
        real(dp), intent(out) :: l2_error, divergence_error

        type(mesh_t) :: mesh
        type(triangle_rt_basis_t) :: basis
        real(dp), allocatable :: dofs(:), eta(:), weights(:), xi(:)
        real(dp) :: determinant, exact_divergence, exact_value(2)
        ...
        do triangle = 1, mesh%data%n_triangles
            ...
            call interpolate_triangle_rt( &
                vertices, degree, quadrature_degree, manufactured_field, &
                dofs, status)
            do point = 1, size(weights)
                ...
                divergence_error = divergence_error + &
                    ...
                    (interpolated_divergence - exact_divergence)**2
            end do
            deallocate(dofs)
        end do
...
end program test_triangle_rt_manufactured_convergence
