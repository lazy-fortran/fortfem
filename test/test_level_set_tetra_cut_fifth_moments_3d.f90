program test_level_set_tetra_cut_fifth_moments_3d
    ...
    real(dp) :: positive_fifth(3, 3, 3, 3, 3), negative_fifth(3, 3, 3, 3, 3)
    real(dp) :: positive_fifth_dot(3, 3, 3, 3, 3), negative_fifth_dot(3, 3, 3, 3, 3)
    real(dp) :: plus_fifth(3, 3, 3, 3, 3), minus_fifth(3, 3, 3, 3, 3)
    real(dp) :: plus_negative_fifth(3, 3, 3, 3, 3)
    real(dp) :: minus_negative_fifth(3, 3, 3, 3, 3)
    real(dp) :: expected_positive(3, 3, 3, 3, 3), expected_parent(3, 3, 3, 3, 3)
    real(dp) :: step
    ...
    call evaluate_level_set_tetra_cut_fifth_moments_3d( &
        vertices, level_values, positive_fifth, negative_fifth, status)
    call record_condition(status == 0 .and. maxval(abs(positive_fifth - &
        expected_positive)) < 1.0e-14_dp, &
        "positive tetra cut integrates quintic monomials exactly")
    call record_condition(maxval(abs(positive_fifth + negative_fifth - &
        expected_parent)) < 1.0e-14_dp, &
        "positive and negative quintic moments conserve the parent tetrahedron")

    call evaluate_level_set_tetra_cut_fifth_moments_3d_jvp( &
        vertices, level_values, vertices_dot, level_values_dot, &
        positive_fifth_dot, negative_fifth_dot, status)
    step = 1.0e-6_dp
    call evaluate_level_set_tetra_cut_fifth_moments_3d( &
        vertices + step*vertices_dot, level_values + step*level_values_dot, &
        plus_fifth, plus_negative_fifth, plus_status)
    call evaluate_level_set_tetra_cut_fifth_moments_3d( &
        vertices - step*vertices_dot, level_values - step*level_values_dot, &
        minus_fifth, minus_negative_fifth, minus_status)
    call record_condition(status == 0 .and. plus_status == 0 .and. &
        minus_status == 0 .and. maxval(abs(positive_fifth_dot - &
        (plus_fifth - minus_fifth)/(2.0_dp*step))) < 1.0e-7_dp .and. &
        maxval(abs(negative_fifth_dot - (plus_negative_fifth - &
        minus_negative_fifth)/(2.0_dp*step))) < 1.0e-7_dp, &
        "tetrahedral quintic cut-moment JVP matches a fixed-topology difference oracle")

    call check_summary("3D level-set tetrahedral quintic cut moments")
                ...
                do third = 1, 3
                    do fourth = 1, 3
                        do fifth = 1, 3
                            ...
                            y_degree = count([first, second, third, fourth, fifth] == 2)
                            z_degree = 5 - x_degree - y_degree
                            moment(first, second, third, fourth, fifth) = simplex_moment( &
...
end program test_level_set_tetra_cut_fifth_moments_3d
