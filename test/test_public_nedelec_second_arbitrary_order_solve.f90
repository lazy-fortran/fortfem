program test_public_nedelec_second_arbitrary_order_solve
    ...
    do degree = 1, 4
        ...
        call check_condition(maximum_error < 2.0e-10_dp, &
            "Public second-kind Nedelec solve reproduces a constant field")
    end do
    call check_summary("Public second-kind Nedelec solve")
...
contains
    ...
    subroutine measure_constant_field_error( &
        ...
        call build_triangle_full_vector_dof_map( &
            ...
            global_count, status)
        if (status /= 0) error stop "second-kind global map failed"
        call initialize_triangle_nedelec_second_kind(degree, basis, status)
        if (status /= 0) error stop "second-kind basis failed"
