program test_triangle_nedelec_load_samples_ad
    ...
    call assemble_triangle_nedelec_vector_load_samples( &
        mesh, order, quadrature_degree, source, load, status)
    call check(status%code == 0, "sampled triangle Nedelec load succeeds")
    call assemble_triangle_nedelec_vector_load_samples_jvp( &
        mesh, order, quadrature_degree, source, source_gradients, &
        vertices_dot, source_dot, load_dot, status)
    call check(status%code == 0, "sampled triangle Nedelec load JVP succeeds")
    call assemble_triangle_nedelec_vector_load_samples( &
        plus_mesh, order, quadrature_degree, source_plus, load_plus, status)
    call assemble_triangle_nedelec_vector_load_samples( &
        minus_mesh, order, quadrature_degree, source_minus, load_minus, status)
    call check(maxval(abs( &
        load_dot - (load_plus - load_minus)/(2.0_dp*step))) < 3.0e-9_dp, &
        "sampled triangle Nedelec JVP matches full Eulerian reassembly")
    ...
    call assemble_triangle_nedelec_vector_load_samples_vjp( &
        mesh, order, quadrature_degree, source, source_gradients, load_bar, &
        vertices_bar, source_bar, status)
    call check(status%code == 0, "sampled triangle Nedelec load VJP succeeds")
    ...
    call check(abs(lhs - rhs) < 3.0e-11_dp*max(1.0_dp, abs(lhs), abs(rhs)), &
        "sampled triangle Nedelec products obey the adjoint identity")
...
end program test_triangle_nedelec_load_samples_ad
