program test_triangle_rt_sampled_state_ad
    ...
    divergence_coefficient = 1.4_dp
    mass_coefficient = 0.9_dp
    divergence_coefficient_dot = -0.11_dp
    ...
    call solve_triangle_rt_sampled_state( &
        mesh, degree, quadrature_degree, divergence_coefficient, &
        mass_coefficient, source, constrained, constrained_values, state, &
        status)
    call check(status == 0, "sampled triangle RT state succeeds")
    call solve_triangle_rt_sampled_state_jvp( &
        mesh, degree, quadrature_degree, divergence_coefficient, &
        mass_coefficient, source, source_gradients, constrained, &
        constrained_values, vertices_dot, divergence_coefficient_dot, &
        mass_coefficient_dot, source_dot, constrained_values_dot, state_dot, &
        status)
    call check(status == 0, "sampled triangle RT state JVP succeeds")
    ...
    call solve_triangle_rt_sampled_state( &
        ...
        mass_coefficient + step*mass_coefficient_dot, source_plus, constrained, &
        constrained_values + step*constrained_values_dot, state_plus, status)
    call solve_triangle_rt_sampled_state( &
        minus_mesh, degree, quadrature_degree, &
        divergence_coefficient - step*divergence_coefficient_dot, &
        mass_coefficient - step*mass_coefficient_dot, source_minus, constrained, &
    ...
    call check(maxval(abs( &
        ...
        "sampled triangle RT state JVP matches full reassembly")
    ...
    call solve_triangle_rt_sampled_state_vjp( &
        mesh, degree, quadrature_degree, divergence_coefficient, &
        mass_coefficient, source, source_gradients, constrained, &
        constrained_values, state, state_bar, vertices_bar, &
        divergence_coefficient_bar, mass_coefficient_bar, source_bar, &
        constrained_values_bar, status)
    call check(status == 0, "sampled triangle RT state VJP succeeds")
    ...
    rhs = sum(vertices_bar*vertices_dot) + &
        ...
        mass_coefficient_bar*mass_coefficient_dot + sum(source_bar*source_dot) + &
        dot_product(constrained_values_bar, constrained_values_dot)
    call check(abs(lhs - rhs) < 8.0e-10_dp*max(1.0_dp, abs(lhs), abs(rhs)), &
        "sampled triangle RT state products obey the adjoint identity")
...
end program test_triangle_rt_sampled_state_ad
