program test_laplace_bem_double_layer
    use check, only: check_condition, check_summary
    use fortfem_boundary, only: assemble_laplace_double_layer_constant
    use fortfem_kinds, only: dp
    implicit none

    real(dp), parameter :: pi = acos(-1.0_dp)
    real(dp) :: matrix(4, 4), panel_end(2, 4), panel_start(2, 4)
    real(dp) :: expected, reversed_matrix(2, 2)
    ...
    call record_condition(status == 0 .and. &
        ...
        "Double layer matches an exact right-angle interaction")

    panel_start(:, 2) = [0.0_dp, 1.0_dp]
    panel_end(:, 2) = [0.0_dp, 0.0_dp]
    call assemble_laplace_double_layer_constant( &
        panel_start(:, 1:2), panel_end(:, 1:2), 24, reversed_matrix, status)
    call record_condition(abs(reversed_matrix(1, 2) + matrix(1, 2)) < &
        2.0e-14_dp, "Reversing a source panel reverses its double layer")
    ...
    do panel = 1, 4
        call record_condition( &
            ...
            "Closed-boundary double layer satisfies K one = -one half")
    end do

    call check_summary("Laplace BEM double-layer")
...
end program test_laplace_bem_double_layer
