program test_fci_quintic_curved_polygon_area
    ...
    real(dp) :: controls(2, 4, vertex_count, cell_count)
    real(dp) :: vertices_dot(2, vertex_count, cell_count)
    real(dp) :: controls_dot(2, 4, vertex_count, cell_count)
    real(dp) :: vertices_bar(2, vertex_count, cell_count)
    real(dp) :: controls_bar(2, 4, vertex_count, cell_count)
    real(dp) :: vertices_plus(2, vertex_count, cell_count)
    real(dp) :: vertices_minus(2, vertex_count, cell_count)
    real(dp) :: controls_plus(2, 4, vertex_count, cell_count)
    real(dp) :: controls_minus(2, 4, vertex_count, cell_count)
    real(dp) :: straight_controls(2, 4, vertex_count, cell_count)
        ...
        do edge = 1, vertex_count
            ...
            do control = 1, 4
                straight_controls(:, control, edge, cell) = &
                    vertices(:, edge, cell) + real(control, dp)/5.0_dp* &
    ...
    call record_condition(status%code == 0, &
        "FCI quintic curved polygon area accepts curved cells")
    do cell = 1, cell_count
    ...
    call record_condition(maxval(abs(areas - expected)) < 4.0e-12_dp, &
        "FCI quintic curved polygon area matches Gauss-Green oracle")
    call record_condition(maxval(abs(areas - straight)) > 1.0e-3_dp, &
        "FCI quintic curved polygon area reflects edge curvature")
    call record_condition(all(areas > 0.0_dp), &
        "FCI quintic curved polygon areas are positive")

    call compute_fci_quintic_curved_polygon_cell_areas_2d_jvp( &
    ...
    call record_condition(maxval(abs(areas_dot - &
        ...
        "FCI quintic curved polygon area JVP matches central differences")

    call compute_fci_quintic_curved_polygon_cell_areas_2d_vjp( &
    ...
    call record_condition(abs(lhs - rhs) < &
        ...
        "FCI quintic curved polygon area VJP satisfies the real dot-product identity")

    call compute_fci_quintic_curved_polygon_cell_areas_2d( &
        vertices, straight_controls, areas_plus, status)
    call record_condition(maxval(abs(areas_plus - straight)) < 4.0e-12_dp, &
        "FCI quintic curved area recovers straight polygon area")

    call check_summary("FCI quintic curved polygon area")
...
contains
    ...
    pure real(dp) function gauss_green_area(cell_vertices, edge_controls) &
            result(area)
        real(dp), intent(in) :: cell_vertices(2, vertex_count)
        real(dp), intent(in) :: edge_controls(2, 4, vertex_count)
...
end program test_fci_quintic_curved_polygon_area
