program test_tetra_rt_load_samples_ad
    ...
    call check(status == 0, "RT load quadrature succeeds")
    ...
    call check(relative_error < 5.0e-8_dp, &
        "sampled RT load JVP matches independent Eulerian reassembly")

    call assemble_tetra_rt_vector_load_samples_vjp( &
        vertices, tetrahedra, degree, quadrature_degree, source_values, &
        source_gradients, load_bar, vertices_bar, source_values_bar, &
        sparse_status)
    call check(sparse_status%code == 0, "sampled RT load VJP succeeds")
    ...
    call check(abs(lhs - rhs) < 3.0e-10_dp*max(1.0_dp, abs(lhs), abs(rhs)), &
        "sampled RT load products satisfy the Eulerian adjoint identity")
...
end program test_tetra_rt_load_samples_ad
