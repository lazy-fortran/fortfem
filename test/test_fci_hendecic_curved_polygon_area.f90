program test_fci_hendecic_curved_polygon_area
    ...
    real(dp) :: controls(2, 10, vertex_count, cell_count)
    real(dp) :: straight_controls(2, 10, vertex_count, cell_count)
    real(dp) :: bad_vertices(2, vertex_count, cell_count)
    real(dp) :: bad_controls(2, 10, vertex_count, cell_count)
    real(dp) :: vertices_dot(2, vertex_count, cell_count)
    real(dp) :: controls_dot(2, 10, vertex_count, cell_count)
    real(dp) :: vertices_bar(2, vertex_count, cell_count)
    real(dp) :: controls_bar(2, 10, vertex_count, cell_count)
    real(dp) :: vertices_plus(2, vertex_count, cell_count)
    real(dp) :: vertices_minus(2, vertex_count, cell_count)
    real(dp) :: controls_plus(2, 10, vertex_count, cell_count)
    real(dp) :: controls_minus(2, 10, vertex_count, cell_count)
    ...
    call compute_fci_hendecic_curved_polygon_cell_areas_2d( &
        vertices, controls, areas, status)
    call record_condition(status%code == 0, &
        "FCI hendecic curved polygon area accepts curved cells")
    do cell = 1, cell_count
    ...
    call record_condition(maxval(abs(areas - expected)) < 8.0e-12_dp, &
        "FCI hendecic curved polygon area matches 11-point Gauss-Green oracle")
    call record_condition(maxval(abs(areas - straight)) > 1.0e-3_dp, &
        "FCI hendecic curved polygon area resolves edge curvature")
    call record_condition(all(areas > 0.0_dp), &
        "FCI hendecic curved polygon areas are positive")

    call compute_fci_hendecic_curved_polygon_cell_areas_2d_jvp( &
    ...
    call record_condition(maxval(abs(areas_dot - &
        ...
        "FCI hendecic curved polygon area JVP matches central differences")

    call compute_fci_hendecic_curved_polygon_cell_areas_2d_vjp( &
    ...
    call record_condition(abs(lhs - rhs) < &
        ...
        "FCI hendecic curved polygon area VJP satisfies the real dot-product identity")

    call compute_fci_hendecic_curved_polygon_cell_areas_2d( &
        vertices, straight_controls, areas_plus, status)
    call record_condition(maxval(abs(areas_plus - straight)) < 8.0e-12_dp, &
        "FCI hendecic curved area recovers straight polygon area")
    ...
    call record_condition(status%code /= 0 .and. all(areas_plus == 0.0_dp), &
        "FCI hendecic curved polygon area rejects degenerate cells")

    call check_summary("FCI hendecic curved polygon area")
...
contains
    ...
    pure real(dp) function gauss_green_area(cell_vertices, edge_controls) &
            result(area)
        real(dp), intent(in) :: cell_vertices(2, vertex_count)
        real(dp), intent(in) :: edge_controls(2, 10, vertex_count)
        ...
        do edge = 1, vertex_count
            ...
            do quadrature = 1, size(gauss_points)
                call hendecic_bezier_point_and_tangent( &
    ...
    pure subroutine hendecic_bezier_point_and_tangent( &
            first, controls, last, parameter, point, tangent)
        real(dp), intent(in) :: first(2), controls(2, 10), last(2), parameter
        real(dp), intent(out) :: point(2), tangent(2)
        real(dp) :: bernstein(0:11), derivative_bernstein(0:10)
        real(dp) :: control_points(2, 0:11), one_minus_parameter
        integer :: index

        one_minus_parameter = 1.0_dp - parameter
        control_points(:, 0) = first
        control_points(:, 1:10) = controls
        control_points(:, 11) = last
        bernstein = [ &
            one_minus_parameter**11, &
            11.0_dp*one_minus_parameter**10*parameter, &
            55.0_dp*one_minus_parameter**9*parameter**2, &
            165.0_dp*one_minus_parameter**8*parameter**3, &
            330.0_dp*one_minus_parameter**7*parameter**4, &
            462.0_dp*one_minus_parameter**6*parameter**5, &
            462.0_dp*one_minus_parameter**5*parameter**6, &
            330.0_dp*one_minus_parameter**4*parameter**7, &
            165.0_dp*one_minus_parameter**3*parameter**8, &
            55.0_dp*one_minus_parameter**2*parameter**9, &
            11.0_dp*one_minus_parameter*parameter**10, parameter**11]
        derivative_bernstein = [ &
            one_minus_parameter**10, &
            10.0_dp*one_minus_parameter**9*parameter, &
            45.0_dp*one_minus_parameter**8*parameter**2, &
            120.0_dp*one_minus_parameter**7*parameter**3, &
            210.0_dp*one_minus_parameter**6*parameter**4, &
            252.0_dp*one_minus_parameter**5*parameter**5, &
            210.0_dp*one_minus_parameter**4*parameter**6, &
            120.0_dp*one_minus_parameter**3*parameter**7, &
            45.0_dp*one_minus_parameter**2*parameter**8, &
            10.0_dp*one_minus_parameter*parameter**9, parameter**10]
        point = 0.0_dp
        tangent = 0.0_dp
        do index = 0, 11
        ...
        do index = 0, 10
            tangent = tangent + 11.0_dp*derivative_bernstein(index)* &
                (control_points(:, index + 1) - control_points(:, index))
        end do
    end subroutine hendecic_bezier_point_and_tangent
...
end program test_fci_hendecic_curved_polygon_area
