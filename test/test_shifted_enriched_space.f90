program test_shifted_enriched_space
    ...
    integer, parameter :: basis_count = 3, point_count = 4
    ...
    real(dp) :: base_dot(basis_count, point_count)
    real(dp) :: level_dot(point_count), anchor_dot(basis_count)
    real(dp) :: enriched_dot(basis_count, point_count)
    real(dp) :: enriched_plus(basis_count, point_count)
    real(dp) :: enriched_bar(basis_count, point_count)
    real(dp) :: base_bar(basis_count, point_count)
    ...
    call record_condition(status%code == 0, "shifted enriched space assembles")
    call record_condition(maxval(abs(enriched_values - expected_values( &
        base_values, level_values, anchor_values))) < 1.0e-14_dp, &
        "shifted enriched space matches its sign matrix oracle")
    ...
    call record_condition(status%code == 0, "shifted enriched space JVP assembles")
    epsilon = 1.0e-7_dp
    call evaluate_shifted_enriched_space( &
    ...
    call record_condition(finite_difference_error < 2.0e-8_dp, &
        "shifted enriched space JVP matches a forward difference")
    ...
    call record_condition(status%code == 0, "shifted enriched space VJP assembles")
    ...
    call record_condition(abs(lhs - rhs) < 2.0e-13_dp, &
        "shifted enriched space VJP satisfies the adjoint identity")

    level_values(2) = 0.0_dp
    call evaluate_shifted_enriched_space( &
        base_values, level_values, anchor_values, enriched_values, status)
    call record_condition(status%code /= 0, "topology-event level values are rejected")

    call check_summary("shifted enriched space")
...
contains
    ...
    subroutine build_system(base, levels, anchors)
        ...
        base = reshape([ &
            ...
            0.7_dp, 0.6_dp, -0.4_dp, 0.8_dp, -0.1_dp, 0.3_dp], shape(base))
    ...
    function expected_values(base, levels, anchors) result(expected)
        ...
        real(dp) :: expected(size(base, 1), size(base, 2))
        integer :: basis, point

        do basis = 1, size(base, 1)
            do point = 1, size(base, 2)
                expected(basis, point) = base(basis, point)* &
...
end program test_shifted_enriched_space
