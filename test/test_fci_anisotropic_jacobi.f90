program test_fci_anisotropic_jacobi
    use check, only: check_condition, check_summary
    use fortfem_feec, only: apply_fci_anisotropic_jacobi_preconditioner
    ...
    real(dp), parameter :: residual(6) = [6.0_dp, 3.0_dp, 9.0_dp, 10.0_dp, &
        8.0_dp, 15.0_dp]
    real(dp), parameter :: expected_correction(6) = [ &
        2.0_dp, 1.0_dp, 3.0_dp, 2.0_dp, 2.0_dp, 3.0_dp]
    real(dp) :: correction(6)
    ...
    call check_condition(status%code == 0, &
        "FCI anisotropic Jacobi accepts dissipative plane blocks")
    call check_condition(maxval(abs(correction - expected_correction)) < &
        1.0e-14_dp, "FCI anisotropic Jacobi matches diagonal division")
    ...
    call apply_fci_anisotropic_jacobi_preconditioner( &
        ...
        correction(:5), status)
    call check_condition(status%code /= 0, &
        "FCI anisotropic Jacobi rejects an incompatible correction")
    call check_summary("FCI anisotropic Jacobi preconditioner")
...
end program test_fci_anisotropic_jacobi
