...
ingredients. The pointwise gradient value, JVP, and VJP used by the matrix-free
action are generated by the pinned FortSym revision; the focused test checks
the JVP against a central difference and the VJP against a real adjoint
identity. This API deliberately does not copy PARALLAX implementation code.
