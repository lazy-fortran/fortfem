...
The current implementation baseline is FortFEM `88e7cbc` with 200 passing
...
| Arbitrary-order 2D FEEC | Triangle H1, first/second-kind H(curl), RT/BDM H(div), and DG through order four, with orientations, commuting projections, sparse assembly, convergence tests, public symbolic first-kind H(curl) solves through order four, and public symbolic RT0–RT3 H(div) solves | High-level BDM dispatch and exhaustive mixed solves above RT1-DG1 |
...
mixed high-level PDE solves and public BDM dispatch remain absent.
