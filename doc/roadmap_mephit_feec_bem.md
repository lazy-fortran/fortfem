...
- The conforming electromagnetic trace-space and integral-equation analysis of
  [Buffa and Hiptmair](https://people.math.ethz.ch/~hiptmair/Courses/CEM/BUH03.pdf),
  including the Maxwell jump relations, MFIE, and resonance-safe CFIE
  requirements.
- The barycentric dual complex of
  [Buffa and Christiansen](https://doi.org/10.1090/S0025-5718-07-01965-5)
  and the maintained
...
| Isogeometric FEEC | Nonuniform open B-spline evaluation and analytical derivatives; exact coefficient derivative maps; 2D H1--H(curl)--L2 and 3D H1--H(curl)--H(div)--L2 tensor-product de Rham incidence complexes; direct FortSparse 2D gradient/curl and 3D gradient/curl/divergence incidence assembly; rational tensor-product surface and volume geometry with analytical Jacobians; covariant H1/H(curl), contravariant H(div), and determinant-scaled L2 physical pullbacks; orientation-aware 2D multipatch H1 and tangential H(curl) trace maps and sparse two-patch quotient-complex assembly, including rotated/reversed interfaces, trace-knot compatibility checks, a commuting discrete-gradient oracle, and global `curl(grad)=0`; orientation-derived 3D face H1, tangential H(curl), and normal H(div) maps plus sparse two-volume quotient-complex assembly for arbitrary coordinate faces with tangential swaps/reversals, trace-knot compatibility checks, commuting gradient/curl trace oracles, and global `curl(grad)=0`, `div(curl)=0`; span-local FortSparse 2D and 3D H1 mass/diffusion assembly with scalar, coefficient-field, and anisotropic 2D physical coefficients; named cylindrical Grad--Shafranov and poloidal--toroidal Fourier Laplacian blocks; exact complex `i*n` toroidal derivatives and retained-mode `p+q=n` convolution of the energy-skew poloidal bracket; the published cylindrical JOREK magnetic-flux weak residual and analytical JVP; a mass-norm-preserving, time-reversible implicit-midpoint/Cayley poloidal-flux propagator with retained sparse factorization and upstream Strang/Yoshida geometric composition; physical 2D H(curl) mass/curl-curl, H(div) mass/div-div, and L2 mass assembly; physical 3D H(curl) mass/curl-curl, H(div) mass/div-div, L2 mass, and weak divergence/adjoint-divergence assembly; and algebraic weak gradient/curl and adjoint compositions using upstream sparse products/transposes, covered by nonuniform-knot polynomial-reproduction, dense and sparse exact-sequence in 2D/3D, affine surface/volume maps, invariant-pairing, exact scalar/vector/flux/density constant-mass, exact 3D affine-box volume, coordinate energy, H(curl)/H(div) constant-vector mass, rigid-rotation curl energy, Piola divergence energy and adjoint duality, affine, anisotropic, mixed-gradient/curl duality, affine-flux divergence, affine bracket action and exact bracket skewness, analytical cylindrical `1/R` Grad--Shafranov energy, coefficient-weighted affine-density integration, toroidal mode-two `4 log(2)` energy, exact nonlinear Fourier-triad and central-difference JVP oracles, and long-time/reversibility time-propagator oracles | General multi-patch graph assembly and JOREK's remaining electric-potential/vorticity/density/temperature/parallel-velocity residual/Jacobian and propagator suite |
| Exact nonreflecting maps | FFT planar, circular, and spherical scalar Helmholtz DtN kernels; arbitrary-polygon P1/P0 acoustic displacement-to-pressure NtD, its work-conjugate vector weak form, and a monolithic complex P1 elasticity solve through a resonance-safe combined Calderon equation; biperiodic planar Maxwell strong and weak capacity operators; automatic arbitrary-order tetrahedral Nedelec trace sampling and pulled-back planar capacity blocks; spherical TE/TM Maxwell capacity map; a mixed RWG/RBC weak Maxwell trace-to-flux map composed from exact-curved-torus EFIE, MFIE, and mass forms with assembled and matrix-free JVP/VJP actions; scalar Helmholtz and complex elastic-acoustic weak forms | Pointwise reconstruction, geometry derivatives, resonance-safe regularization, and general curved-surface Maxwell DtN maps |
...
FortFEM's iterative compatibility surface now reexports `fortsparse` CSC
storage and calls its triplet constructor and matrix-vector product. The
internal CSR arrays and CSR-to-CSC conversion were removed after direct,
PCG, and residual-equivalence tests passed. Direct real and complex solves
use `fortsparse` with the permissive in-process SuperLU backend. Several
legacy scalar PDE assemblers still form dense global matrices before solving.
...
converges to the independent outgoing Hankel NtD eigenvalue. A consistent
vector weak form maps nodal displacement to panel-normal motion and nodal
pressure back to structural traction. It reuses one FortSparse factorization
for all boundary basis responses and converges quadratically to the
analytical Hankel traction. The formulation is provenance-matched to
`paper_acoustics` revision `6300ab0` and Burton and Miller,
*Proc. R. Soc. A* 323 (1971), 201--210,
...
`paper_acoustics` Fortran result. Complex P1 elasticity also integrates the
curved NtD weak form and solves the coupled system with FortSparse. Its
manufactured rigid-translation problem uses an independently evaluated
Hankel pressure traction and converges quadratically. The planar and curved
solvers share one dynamic-elasticity assembly and constrained FortSparse
solve. Higher-order traces and fast boundary operators remain.
...
| `real_gmres_operator` and `complex_gmres_operator` | FortFEM's public real restarted-GMRES solver and retained-tree complex Helmholtz boundary integral solves without private duplicate Arnoldi implementations. |
...
The public real GMRES compatibility API now delegates to FortNum's
reorthogonalized matrix-free Arnoldi implementation; the former private
FortFEM GMRES implementation has been deleted. FortNum verifies both real and
complex paths against independent dense LU solutions of nonsymmetric systems.
...
set have been deleted. Iterative solvers now consume the same `fortsparse`
CSC type. New vector finite-element assembly emits triplets directly into
`fortsparse`; remaining dense scalar assemblers should follow. The final
solver layer should accept an
...
- a biperiodic planar Maxwell capacity operator uses two-dimensional
  ...
  Its reciprocal weak form matches the independently applied strong operator,
  and the complex Nedelec solver accepts the resulting dense boundary block
  on selected global trace degrees of freedom.
- tetrahedral first-kind Nedelec traces of orders one through four now sample
  automatically on a selected incidence-one planar boundary. The compact
  pulled-back capacity block contains only boundary edge and face moments and
  reproduces the analytical normal-incidence Maxwell eigenvalue at every
  implemented order.
...
RT3-DG3. Tetrahedral Nedelec curl-curl-plus-mass, tensor-weighted curl-curl,
RT div-div-plus-mass, and H1 diffusion-reaction/Poisson solves now cover every
...
quartic solution at degree four. The anisotropic `paper_magnetic` box now
attains the membrane-series vector potential and differentiated magnetic
...
General multi-field block composition remains an API gap rather than a
missing finite-element family. The three-dimensional RT--DG divergence
...
convergence curve without checking generated images into git. It also
publishes the monolithic elastic displacement and its quadratic convergence
to a manufactured rigid translation driven by the analytical outgoing Hankel
traction.
...
Item 2 is implemented for affine Laplace and outgoing Helmholtz P0 surfaces:
marked edges receive conforming red/green refinement, a uniformly refined
auxiliary space supplies local fine-grid residual indicators, and Dörfler
marking selects a minimal largest-indicator set. The prolate-spheroid and
outgoing-sphere gallery loops demonstrate decreasing capacity and radiated
field errors against independent closed forms. Extension to Maxwell and
higher-order trace spaces remains.
...
conservation. Exact parametric torus panels now provide curved Laplace and
Helmholtz \(V/K\) operators and exterior DtN solves. Their P0 fluxes converge
to independent half-integer-Legendre Laplace and outgoing-point-source
Helmholtz traces. The Helmholtz implementation subtracts the Laplace
singularity and integrates only the smooth wave correction. Exact-curved
off-surface reconstruction also converges to the outgoing Helmholtz field
when supplied with its independent nodal Cauchy trace. The curved Laplace
operator now includes the Galerkin-transpose \(K'\) and surface-curl
regularization of \(W\); the second exterior Calderon equation converges for
the independent toroidal harmonic. The curved Helmholtz \(K'\)/\(W\) blocks
use the same surface-curl regularization and their second Calderon equation
converges to an independent outgoing-source trace. Higher-order geometry and
...
The symmetric Laplace Costabel-Han system now couples that affine P1 volume
to the full exact-curved-torus Calderon operator. A centerline point source
converges to the analytical boundary potential. Its integrated exterior flux
improves from \(-0.9776\) to \(-0.9813\); the remaining two-percent defect is
the measured mismatch between the affine FEM boundary and exact BEM geometry,
not claimed as exact conservation.

The complex symmetric Helmholtz Costabel-Han system uses the same solid-torus
volume and the full curved Helmholtz Calderon operator. For a centerline
source, its boundary potential converges to the independently evaluated
