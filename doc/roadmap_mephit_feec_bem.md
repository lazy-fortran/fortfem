...
| Isogeometric FEEC | Nonuniform open B-spline evaluation and analytical derivatives; exact coefficient derivative maps; 2D H1--H(curl)--L2 and 3D H1--H(curl)--H(div)--L2 tensor-product de Rham incidence complexes; direct FortSparse 2D gradient/curl and 3D gradient/curl/divergence incidence assembly; rational tensor-product surface and volume geometry with analytical Jacobians; covariant H1/H(curl), contravariant H(div), and determinant-scaled L2 physical pullbacks; span-local FortSparse 2D and 3D H1 mass/diffusion assembly with scalar and anisotropic 2D physical coefficients; named cylindrical Grad--Shafranov and poloidal--toroidal Fourier Laplacian blocks; an energy-conserving skew poloidal bracket; physical 2D H(curl) mass/curl-curl, H(div) mass/div-div, and L2 mass assembly; physical 3D H(div) mass/div-div, L2 mass, and weak divergence/adjoint-divergence assembly; and algebraic weak gradient/curl and adjoint compositions using upstream sparse products/transposes, covered by nonuniform-knot polynomial-reproduction, dense and sparse exact-sequence in 2D/3D, affine surface/volume maps, invariant-pairing, exact scalar/vector/flux/density constant-mass, exact 3D affine-box volume, coordinate energy, H(div) constant-vector mass, Piola divergence energy and adjoint duality, affine, anisotropic, mixed-gradient/curl duality, rigid-rotation curl, affine-flux divergence, affine bracket action and exact bracket skewness, analytical cylindrical `1/R` Grad--Shafranov energy, and toroidal mode-two `4 log(2)` energy oracles | 3D H(curl) mass and curl-curl weak forms, multipatch conformity, and JOREK's coupled nonlinear MHD operator suite |
