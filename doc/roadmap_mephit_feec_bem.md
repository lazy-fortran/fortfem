...
| Isogeometric FEEC | Nonuniform open B-spline evaluation and analytical derivatives; exact coefficient derivative maps; 2D H1--H(curl)--L2 and 3D H1--H(curl)--H(div)--L2 tensor-product de Rham incidence complexes; direct FortSparse 2D gradient/curl and 3D gradient/curl/divergence incidence assembly; rational tensor-product surface and volume geometry with analytical Jacobians; covariant H1/H(curl), contravariant H(div), and determinant-scaled L2 physical pullbacks; span-local FortSparse H1 mass/diffusion assembly with scalar and anisotropic physical coefficients; physical H(curl) mass/curl-curl, H(div) mass/div-div, and L2 mass assembly; and algebraic weak gradient/curl and adjoint compositions using upstream sparse products/transposes, covered by nonuniform-knot polynomial-reproduction, dense and sparse exact-sequence in 2D/3D, affine surface/volume maps, invariant-pairing, exact scalar/vector/flux/density constant-mass, affine, anisotropic, mixed-gradient/curl duality, rigid-rotation curl, and affine-flux divergence energies, and analytical cylindrical `1/R` Grad--Shafranov energy oracles | 3D sparse weak forms, multipatch conformity, and JOREK's nonlinear poloidal Bézier/Fourier operator suite |
