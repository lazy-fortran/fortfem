...
| Isogeometric FEEC | Nonuniform open B-spline evaluation and analytical derivatives; exact coefficient derivative maps; 2D H1--H(curl)--L2 and 3D H1--H(curl)--H(div)--L2 tensor-product de Rham incidence complexes; rational tensor-product surface geometry with analytical Jacobians; covariant H1/H(curl), contravariant H(div), and determinant-scaled L2 physical pullbacks; span-local FortSparse H1 mass/diffusion assembly with scalar and anisotropic physical coefficients; direct sparse gradient/curl incidence assembly; physical H(curl) mass/curl-curl and H(div) mass/div-div assembly; and algebraic H1-to-H(curl) weak-gradient and adjoint composition using upstream sparse products/transposes, covered by nonuniform-knot polynomial-reproduction, dense and sparse exact-sequence, affine-map, invariant-pairing, exact scalar/vector/flux constant-mass, affine, anisotropic, mixed-gradient duality, rigid-rotation curl, and affine-flux divergence energies, and analytical cylindrical `1/R` Grad--Shafranov energy oracles | Remaining mixed weak forms, rational volume geometry, multipatch conformity, and JOREK's nonlinear poloidal Bézier/Fourier operator suite |
