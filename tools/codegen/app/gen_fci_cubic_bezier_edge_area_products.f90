program gen_fci_cubic_bezier_edge_area_products
    ...
    type(expr_t) :: variables(8), variables_dot(8), edge_area(1)
    type(expr_t) :: edge_area_bar(1), edge_area_jvp(1), edge_area_vjp(8)
    type(expr_t) :: x_coeff(0:3), y_coeff(0:3), curve_parameter
    ...
    x_coeff(1) = three*(variables(3) - variables(1))
    x_coeff(2) = three*(variables(1) - two*variables(3) + variables(5))
    x_coeff(3) = variables(7) - variables(1) + &
        three*(variables(3) - variables(5))
    y_coeff(0) = variables(2)
    y_coeff(1) = three*(variables(4) - variables(2))
    y_coeff(2) = three*(variables(2) - two*variables(4) + variables(6))
    y_coeff(3) = variables(8) - variables(2) + &
        three*(variables(4) - variables(6))
    edge_x = x_coeff(0)
    edge_y = y_coeff(0)
    do degree = 1, 3
    ...
    ! Keep the explicit polynomial construction in the generator as a readback
    ! check against the differentiated parametric curve.
    if (edge_x%node_count() < 1 .or. derivative_x%node_count() < 1 .or. &
        edge_y%node_count() < 1 .or. derivative_y%node_count() < 1) then
        error stop "cubic Bezier generator produced an empty curve"
    end if
    call simplify_all(edge_area)

    call initialize_spec(spec, "generated_fci_cubic_bezier_edge_area", &
        "fortfem_generated_fci_cubic_bezier_edge_area", 8, 1)
    ...
    call initialize_spec(spec, "generated_fci_cubic_bezier_edge_area_jvp", &
        "fortfem_generated_fci_cubic_bezier_edge_area_jvp", 16, 1)
    spec%args = jvp_arguments()
    spec%outputs(1) = str("edge_area_dot")
    jvp_code = chars(emit_kernel(edge_area_jvp, spec))

    call initialize_spec(spec, "generated_fci_cubic_bezier_edge_area_vjp", &
        "fortfem_generated_fci_cubic_bezier_edge_area_vjp", 9, 8)
    ...
    open (newunit=unit, file=generated_path( &
        "fortfem_fci_cubic_bezier_edge_area_products.f90"), &
        status="replace", action="write", iostat=ios)
    if (ios /= 0) error stop "cannot write FCI cubic Bezier edge products"
...
contains
    ...
    function jvp_arguments() result(arguments)
        ...
        arguments(1:8) = primal_arguments()
        arguments(9:16) = [str("x_start_dot"), str("y_start_dot"), &
    ...
    subroutine initialize_spec(kernel_spec, name, module_name, argument_count, &
        ...
        kernel_spec%generator = str("gen_fci_cubic_bezier_edge_area_products")
...
end program gen_fci_cubic_bezier_edge_area_products
