program gen_fci_quartic_bezier_edge_area_products
    ...
    type(expr_t) :: variables(10), variables_dot(10), edge_area(1)
    type(expr_t) :: edge_area_bar(1), edge_area_jvp(1), edge_area_vjp(10)
    type(expr_t) :: x_coeff(0:4), y_coeff(0:4), curve_parameter
    type(expr_t) :: half, two, three, four, six
    ...
    x_coeff(1) = four*(variables(3) - variables(1))
    x_coeff(2) = six*(variables(1) - two*variables(3) + variables(5))
    x_coeff(3) = four*(-variables(1) + three*variables(3) - &
        three*variables(5) + variables(7))
    x_coeff(4) = variables(9) + variables(1) - four*variables(3) + &
        six*variables(5) - four*variables(7)
    y_coeff(0) = variables(2)
    y_coeff(1) = four*(variables(4) - variables(2))
    y_coeff(2) = six*(variables(2) - two*variables(4) + variables(6))
    y_coeff(3) = four*(-variables(2) + three*variables(4) - &
        three*variables(6) + variables(8))
    y_coeff(4) = variables(10) + variables(2) - four*variables(4) + &
        six*variables(6) - four*variables(8)
    edge_x = x_coeff(0)
    edge_y = y_coeff(0)
    do degree = 1, 4
    ...
    if (edge_x%node_count() < 1 .or. derivative_x%node_count() < 1 .or. &
        ...
        error stop "quartic Bezier generator produced an empty curve"
    end if
    call simplify_all(edge_area)

    call initialize_spec(spec, "generated_fci_quartic_bezier_edge_area", &
        "fortfem_generated_fci_quartic_bezier_edge_area", 10, 1)
    ...
    call initialize_spec(spec, "generated_fci_quartic_bezier_edge_area_jvp", &
        "fortfem_generated_fci_quartic_bezier_edge_area_jvp", 20, 1)
    spec%args = jvp_arguments()
    spec%outputs(1) = str("edge_area_dot")
    jvp_code = chars(emit_kernel(edge_area_jvp, spec))

    call initialize_spec(spec, "generated_fci_quartic_bezier_edge_area_vjp", &
        "fortfem_generated_fci_quartic_bezier_edge_area_vjp", 11, 10)
    ...
    open (newunit=unit, file=generated_path( &
        "fortfem_fci_quartic_bezier_edge_area_products.f90"), &
        status="replace", action="write", iostat=ios)
    if (ios /= 0) error stop "cannot write FCI quartic Bezier edge products"
...
contains
    ...
    function primal_arguments() result(arguments)
        ...
        arguments = [str("x_start"), str("y_start"), str("control_1_x"), &
            ...
            str("control_3_x"), str("control_3_y"), str("x_end"), &
            str("y_end")]
    ...
    function jvp_arguments() result(arguments)
        ...
        allocate(arguments(20))
        arguments(1:10) = primal_arguments()
        arguments(11:20) = [str("x_start_dot"), str("y_start_dot"), &
    ...
    function vjp_arguments() result(arguments)
        ...
        allocate(arguments(11))
        arguments(1:10) = primal_arguments()
        arguments(11) = str("edge_area_bar")
    ...
    subroutine initialize_spec(kernel_spec, name, module_name, argument_count, &
        ...
        kernel_spec%generator = str("gen_fci_quartic_bezier_edge_area_products")
...
end program gen_fci_quartic_bezier_edge_area_products
