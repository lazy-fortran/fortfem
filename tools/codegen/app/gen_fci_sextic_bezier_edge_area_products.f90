program gen_fci_sextic_bezier_edge_area_products
    ...
    type(expr_t) :: variables(14), variables_dot(14), edge_area(1)
    type(expr_t) :: edge_area_bar(1), edge_area_jvp(1), edge_area_vjp(14)
    type(expr_t) :: x_coeff(0:6), y_coeff(0:6), curve_parameter
    type(expr_t) :: half, two, three, four, five, six, ten, fifteen, twenty
    ...
    fifteen = real_expr(arena, 15.0d0)
    twenty = real_expr(arena, 20.0d0)
    variables = [ &
    ...
    x_coeff(1) = six*(variables(3) - variables(1))
    x_coeff(2) = fifteen*(variables(1) - two*variables(3) + variables(5))
    x_coeff(3) = twenty*(-variables(1) + three*variables(3) - &
        three*variables(5) + variables(7))
    x_coeff(4) = fifteen*(variables(1) - four*variables(3) + &
        six*variables(5) - four*variables(7) + variables(9))
    x_coeff(5) = six*(-variables(1) + five*variables(3) - &
        ten*variables(5) + ten*variables(7) - five*variables(9) + variables(11))
    x_coeff(6) = variables(1) - six*variables(3) + fifteen*variables(5) - &
        twenty*variables(7) + fifteen*variables(9) - six*variables(11) + &
        variables(13)
    y_coeff(0) = variables(2)
    y_coeff(1) = six*(variables(4) - variables(2))
    y_coeff(2) = fifteen*(variables(2) - two*variables(4) + variables(6))
    y_coeff(3) = twenty*(-variables(2) + three*variables(4) - &
        three*variables(6) + variables(8))
    y_coeff(4) = fifteen*(variables(2) - four*variables(4) + &
        six*variables(6) - four*variables(8) + variables(10))
    y_coeff(5) = six*(-variables(2) + five*variables(4) - &
        ten*variables(6) + ten*variables(8) - five*variables(10) + variables(12))
    y_coeff(6) = variables(2) - six*variables(4) + fifteen*variables(6) - &
        twenty*variables(8) + fifteen*variables(10) - six*variables(12) + &
        variables(14)
    edge_x = x_coeff(0)
    edge_y = y_coeff(0)
    do degree = 1, 6
    ...
    if (edge_x%node_count() < 1 .or. derivative_x%node_count() < 1 .or. &
        ...
        error stop "sextic Bezier generator produced an empty curve"
    end if
    call simplify_all(edge_area)

    call initialize_spec(spec, "generated_fci_sextic_bezier_edge_area", &
        "fortfem_generated_fci_sextic_bezier_edge_area", 14, 1)
    ...
    call initialize_spec(spec, "generated_fci_sextic_bezier_edge_area_jvp", &
        "fortfem_generated_fci_sextic_bezier_edge_area_jvp", 28, 1)
    spec%args = jvp_arguments()
    spec%outputs(1) = str("edge_area_dot")
    jvp_code = chars(emit_kernel(edge_area_jvp, spec))

    call initialize_spec(spec, "generated_fci_sextic_bezier_edge_area_vjp", &
        "fortfem_generated_fci_sextic_bezier_edge_area_vjp", 15, 14)
    ...
    open (newunit=unit, file=generated_path( &
        "fortfem_fci_sextic_bezier_edge_area_products.f90"), &
        status="replace", action="write", iostat=ios)
    if (ios /= 0) error stop "cannot write FCI sextic Bezier edge products"
...
contains
    ...
    function primal_arguments() result(arguments)
        ...
        arguments = [str("x_start"), str("y_start"), str("control_1_x"), &
            ...
            str("control_4_y"), str("control_5_x"), str("control_5_y"), &
            str("x_end"), str("y_end")]
    ...
    function jvp_arguments() result(arguments)
        ...
        allocate(arguments(28))
        arguments(1:14) = primal_arguments()
        arguments(15:28) = [str("x_start_dot"), str("y_start_dot"), &
    ...
    function vjp_arguments() result(arguments)
        ...
        allocate(arguments(15))
        arguments(1:14) = primal_arguments()
        arguments(15) = str("edge_area_bar")
    ...
    subroutine initialize_spec(kernel_spec, name, module_name, argument_count, &
        ...
        kernel_spec%generator = str("gen_fci_sextic_bezier_edge_area_products")
...
end program gen_fci_sextic_bezier_edge_area_products
