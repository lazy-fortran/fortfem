program gen_helmholtz_bem_kernel_products
    use fortsym_arena, only: arena_t
    use fortsym_engine, only: engine_result_t
    use fortsym_engine_native, only: make_native_engine, native_engine_t
    use fortsym_expr, only: cos, expr_t, operator(+), operator(-), &
    ...
    outputs = [amplitude*cos(phase), amplitude*sin(phase)]
    ...
    call initialize_spec( &
        spec, "generated_helmholtz_single_layer_integrand", &
        "fortfem_generated_helmholtz_single_layer_integrand", 10, 2)
    spec%args = primal_arguments()
    spec%outputs = [str("value_real"), str("value_imag")]
    primal_code = chars(emit_kernel(outputs, spec))

    call initialize_spec( &
        spec, "generated_helmholtz_single_layer_integrand_jvp", &
        "fortfem_generated_helmholtz_single_layer_integrand_jvp", 19, 2)
    ...
    call initialize_spec( &
        spec, "generated_helmholtz_single_layer_integrand_vjp", &
        "fortfem_generated_helmholtz_single_layer_integrand_vjp", 12, 9)
    spec%args(1:10) = primal_arguments()
    ...
    open (newunit=unit, &
        file=generated_path("fortfem_helmholtz_bem_kernel_products.f90"), &
        status="replace", action="write", iostat=ios)
    if (ios /= 0) error stop "cannot write Helmholtz BEM kernel products"
...
contains
    ...
    subroutine initialize_spec( &
        ...
        kernel_spec%generator = str("gen_helmholtz_bem_kernel_products")
...
end program gen_helmholtz_bem_kernel_products
