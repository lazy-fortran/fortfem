program gen_fci_sextic_lagrange
    ...
    integer, parameter :: node_count = 7
    ...
    call initialize_spec(spec, "generated_fci_sextic_lagrange_weights", &
        "fortfem_generated_fci_sextic_lagrange", 8, 7)
    ...
    variables_dot(8) = sym(arena, "node_7_dot")
    ...
    output_bar(7) = sym(arena, "weight_7_bar")
    ...
    call initialize_spec(spec, "generated_fci_sextic_lagrange_weights_jvp", &
        "fortfem_generated_fci_sextic_lagrange_jvp", 16, 7)
    spec%args(1) = str("target")
    spec%args(2) = str("node_1")
    spec%args(3) = str("node_2")
    spec%args(4) = str("node_3")
    ...
    spec%args(7) = str("node_6")
    spec%args(8) = str("node_7")
    spec%args(9) = str("target_dot")
    spec%args(10) = str("node_1_dot")
    spec%args(11) = str("node_2_dot")
    spec%args(12) = str("node_3_dot")
    spec%args(13) = str("node_4_dot")
    spec%args(14) = str("node_5_dot")
    spec%args(15) = str("node_6_dot")
    spec%args(16) = str("node_7_dot")
    ...
    spec%outputs(7) = str("weight_7_dot")
    jvp_code = chars(emit_kernel(outputs_jvp, spec))

    call initialize_spec(spec, "generated_fci_sextic_lagrange_weights_vjp", &
        "fortfem_generated_fci_sextic_lagrange_vjp", 15, 8)
    spec%args(1) = str("target")
    spec%args(2) = str("node_1")
    spec%args(3) = str("node_2")
    spec%args(4) = str("node_3")
    ...
    spec%args(7) = str("node_6")
    spec%args(8) = str("node_7")
    spec%args(9) = str("weight_1_bar")
    spec%args(10) = str("weight_2_bar")
    spec%args(11) = str("weight_3_bar")
    spec%args(12) = str("weight_4_bar")
    spec%args(13) = str("weight_5_bar")
    spec%args(14) = str("weight_6_bar")
    spec%args(15) = str("weight_7_bar")
    ...
    spec%outputs(8) = str("node_7_bar")
    vjp_code = chars(emit_kernel(outputs_vjp, spec))

    open (newunit=unit, file=generated_path( &
        "fortfem_fci_sextic_lagrange.f90"), status="replace", &
        action="write", iostat=ios)
    if (ios /= 0) error stop "cannot write FCI sextic Lagrange products"
...
contains
    ...
    subroutine initialize_spec(kernel_spec, name, module_name, argument_count, &
        ...
        kernel_spec%generator = str("gen_fci_sextic_lagrange")
...
end program gen_fci_sextic_lagrange
