program gen_fci_septic_bezier_edge_area_products
    ...
    integer, parameter :: polynomial_degree = 7
    ...
    if (edge_x%node_count() < 1 .or. derivative_x%node_count() < 1 .or. &
        ...
        error stop "septic Bezier generator produced an empty curve"
    end if
    call simplify_all(edge_area)

    call initialize_spec(spec, "generated_fci_septic_bezier_edge_area", &
        "fortfem_generated_fci_septic_bezier_edge_area", variable_count, 1)
    ...
    call initialize_spec(spec, "generated_fci_septic_bezier_edge_area_jvp", &
        "fortfem_generated_fci_septic_bezier_edge_area_jvp", &
        2*variable_count, 1)
    spec%args = jvp_arguments()
    spec%outputs(1) = str("edge_area_dot")
    jvp_code = chars(emit_kernel(edge_area_jvp, spec))

    call initialize_spec(spec, "generated_fci_septic_bezier_edge_area_vjp", &
        "fortfem_generated_fci_septic_bezier_edge_area_vjp", &
    ...
    open (newunit=unit, file=generated_path( &
        "fortfem_fci_septic_bezier_edge_area_products.f90"), &
        status="replace", action="write", iostat=ios)
    if (ios /= 0) error stop "cannot write FCI septic Bezier edge products"
...
contains
    ...
    subroutine initialize_spec(kernel_spec, name, module_name, argument_count, &
        ...
        kernel_spec%generator = str("gen_fci_septic_bezier_edge_area_products")
...
end program gen_fci_septic_bezier_edge_area_products
