program gen_surface_integral_products
    !! Emit scalar surface-integral products for the runtime ledger wrapper.
    !!
    !! The wrapper owns the fixed-topology array reduction and target subtraction;
    !! FortSym owns the pointwise product and its exact tangent/reverse products.
    ...
    type(expr_t) :: variables(2), variables_dot(2), outputs(1)
    type(expr_t) :: outputs_jvp(1), output_bar(1), outputs_vjp(2)
    ...
    variables = [sym(arena, "sample"), sym(arena, "weight")]
    variables_dot = [sym(arena, "sample_dot"), sym(arena, "weight_dot")]
    outputs(1) = variables(1)*variables(2)
    ...
    call initialize_spec(spec, "generated_surface_integral_contribution", &
        "fortfem_generated_surface_integral_contribution", 2, 1)
    spec%args = [str("sample"), str("weight")]
    spec%outputs = [str("contribution")]
    value_code = chars(emit_kernel(outputs, spec))

    call initialize_spec(spec, "generated_surface_integral_contribution_jvp", &
        "fortfem_generated_surface_integral_contribution_jvp", 4, 1)
    spec%args = [str("sample"), str("weight"), str("sample_dot"), &
        str("weight_dot")]
    spec%outputs = [str("contribution_dot")]
    jvp_code = chars(emit_kernel(outputs_jvp, spec))

    call initialize_spec(spec, "generated_surface_integral_contribution_vjp", &
        "fortfem_generated_surface_integral_contribution_vjp", 3, 2)
    spec%args = [str("sample"), str("weight"), str("contribution_bar")]
    spec%outputs = [str("sample_bar"), str("weight_bar")]
    vjp_code = chars(emit_kernel(outputs_vjp, spec))

    open (newunit=unit, file=generated_path( &
        "fortfem_surface_integral_products.f90"), status="replace", &
        action="write", iostat=ios)
    if (ios /= 0) error stop "cannot write surface-integral products"
...
contains
    ...
    subroutine initialize_spec(kernel_spec, name, module_name, argument_count, &
        ...
        kernel_spec%generator = str("gen_surface_integral_products")
...
end program gen_surface_integral_products
